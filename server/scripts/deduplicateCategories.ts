...
async function main() {
  ...
  console.log(
    `Connecting to MongoDB at ${mongoUri.replace(/:([^:@]+)@/, ':****@')} (DB: ${dbName})`
  );
  ...
  try {
    for (const [ptName, enName] of Object.entries(CATEGORY_MAPPING)) {
      ...
      console.log(
        `🔍 Found duplicate "${ptName}" (ID: ${duplicateCategory.id}) with ${duplicateCategory.tags.length} tags.`
      );
      ...
      const updateResult = await presetsCollection.updateMany(
        {
          $or: [{ mockupCategoryId: duplicateId }, { mockupCategoryId: new ObjectId(duplicateId) }],
        },
