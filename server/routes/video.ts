    ...
    try {
      ...
      try {
        ...
        if (indexError.code !== 85 && !indexError.message?.includes('already exists')) {
          console.warn(
            `${logPrefix} [LOCK] Warning: Could not create TTL index:`,
            indexError.message
          );
        }
      }
      ...
      try {
        ...
        if (creditsDeducted && !isAdmin) {
          console.log(
            `${logPrefix} [REFUND] Attempting to refund credits due to generation failure`,
            {
              userId: req.userId,
      ...
      const isUri =
        videoBase64 && (videoBase64.startsWith('http://') || videoBase64.startsWith('https://'));
      ...
      if (canvasId && isR2Configured() && isDataUrl) {
        try {
          ...
          console.warn(
            `${logPrefix} [R2 UPLOAD] Continuing with original format fallback due to R2 upload failure`
          );
        }
      } else {
        if (!canvasId) {
          ...
          console.log(
            `${logPrefix} [R2 UPLOAD] Skipping R2 upload - video is URI, returning as-is`,
            {
              userId: req.userId,
              requestId,
      ...
      // 💾 CACHE SET
      await redisClient
        .setex(cacheKey, CACHE_TTL.VIDEO_GEN, JSON.stringify(response))
        .catch(() => {});
