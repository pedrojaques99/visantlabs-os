...
import { deriveLogoRules, type ClearSpaceModule } from '../lib/brand/logoRules.js';
...
import {
  runBrandHealth,
  isBrandEmpty,
  buildEmptyBrandHealthReport,
  BrandHealthContextError,
} from '../lib/brandHealth.js';
...
/**
 * Whether this brand's own copy reads as Portuguese — picks which of the two
 * names the vision pass produced gets used as an asset label. Same cheap
 * diacritic test `buildEmptyBrandHealthReport` uses; a brand writing in
 * Portuguese should not get its media kit labelled in English.
 */
function brandLanguageIsPt(g: { identity?: any; guidelines?: any; strategy?: any }): boolean {
  const sample = [
    (g.identity as any)?.description,
    (g.identity as any)?.tagline,
    (g.guidelines as any)?.voice,
    typeof (g.strategy as any)?.manifesto === 'string' ? (g.strategy as any).manifesto : '',
  ]
    .filter((s) => typeof s === 'string')
    .join(' ');
  return /[áàâãéêíóôõúç]/i.test(sample);
}

/**
 * True when a Figma variable/style is named after its own value — "EEEEEE",
 * "#2C352F", "FFF". A name like that carries no role, so importing it produces a
 * palette entry that tells generation nothing.
 */
function isHexNamed(name: string | undefined): boolean {
  const n = String(name || '')
    .trim()
    .replace(/^#/, '');
  return /^[0-9a-f]{3,8}$/i.test(n);
}

/**
 * What a figma-sync would actually be worth on this file, stated BEFORE it runs.
 *
 * The variables path is the face of the integration, but most real files have no
 * variable system at all — the Arbolt file has three, all named by hex. Running
 * the sync there imports almost nothing, and the honest read of "nothing
 * happened" is that the integration is weak, when the value was in the file's
 * text and pieces the whole time. Say which path pays before spending the call.
 */
function figmaSyncVerdict(tokens: { colors: any[]; typography: any[]; components: any[] }): {
  recommended: 'variables' | 'text';
  colors: number;
  hexNamedColors: number;
  typography: number;
  components: number;
  reason: string;
} {
  const colors = tokens.colors || [];
  const hexNamed = colors.filter((c: any) => isHexNamed(c?.name)).length;
  const typography = (tokens.typography || []).length;
  const components = (tokens.components || []).length;

  const thinPalette = colors.length < 3;
  const mostlyHexNamed = colors.length > 0 && hexNamed / colors.length >= 0.5;
  const recommended: 'variables' | 'text' =
    thinPalette || mostlyHexNamed || (colors.length === 0 && typography === 0)
      ? 'text'
      : 'variables';

  let reason: string;
  if (colors.length === 0 && typography === 0) {
    reason =
      "This file has no color variables and no text styles. A variable sync would import nothing — extract the brand from the file's text and pieces instead.";
  } else if (mostlyHexNamed) {
    reason = `${hexNamed} of ${colors.length} color variables are named after their own hex value (e.g. "EEEEEE"). Names like that carry no role, so importing them adds swatches without meaning. This file has no real token system — the brand lives in its text and pieces.`;
  } else if (thinPalette) {
    reason = `Only ${colors.length} color variable(s) found. Too thin to be this brand's palette — the brand is more likely expressed in the file's text and pieces.`;
  } else {
    reason = `${colors.length} named color variables and ${typography} text styles — this file has a real token system, so the variable sync is the right path.`;
  }

  return {
    recommended,
    colors: colors.length,
    hexNamedColors: hexNamed,
    typography,
    components,
    reason,
  };
...
/** Cap on files per direct-upload batch — keeps one request from minting hundreds of URLs. */
const MAX_MEDIA_BATCH = 50;

/** Best-effort content type from an explicit value or the filename extension. */
function contentTypeFor(explicit: unknown, filename: unknown): string {
...
/**
 * Pull the bytes behind a stored logo so its rules can be re-derived without
 * asking the user to re-upload. Returns undefined rather than throwing — every
 * caller treats measurement as best-effort.
 */
async function fetchLogoBytes(url?: string): Promise<Buffer | undefined> {
      ...
      batch.map(async ({ asset, kind }) => {
        try {
          ...
          if (result) {
            ...
            // Replace machine debris ("Frame 4836", "IMG_2043.png") with the name
            // the vision pass already produced. A label a person wrote is never
            // touched — only labels that were never really labels.
            const suggested = brandLanguageIsPt(g)
              ? result.analysis.namePt || result.analysis.name
              : result.analysis.name || result.analysis.namePt;
            if (suggested && isAutoGeneratedLabel(asset.label)) {
              asset.label = suggested;
            }
  ...
  try {
    ...
    const updated = await prisma.brandGuideline.update({
      ...
      data: {
        ...
        // Assets count toward completeness — restate it here or the score stays
        // frozen at whatever the last PUT left behind.
        extraction: recomputeExtraction({ ...(guideline as any), media: updatedMedia }) as any,
      },
    });
...
// POST /api/brand-guidelines/:id/media/upload-urls — mint direct-to-R2 PUT URLs
//
// The base64 route above makes the caller carry the bytes. Through MCP that
// means the file crosses the model context: fourteen 1 MB assets is ~15 MB of
// base64 for something the model never reads, which is enough to push an agent
// out of the product and into writing its own upload script. Here the client
// PUTs straight to R2 and only ids come back through the tool call.
//
// Storage quota is enforced on the DECLARED sizes at mint time and again on the
// REAL sizes at commit — a presigned URL must not be a way around the limit.
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const declaredTotal = files.reduce(
        (sum: number, f: any) => sum + (Number.isFinite(f?.size) ? Math.max(0, f.size) : 0),
        0
      );
      if (declaredTotal > 0) {
        const user = await prisma.user.findUnique({
        ...
        if (!user?.isAdmin) {
          ...
          if (!check.allowed) {
            return res.status(413).json({
              ...
              message: 'This batch would exceed the storage limit for your plan.',
              ...check,
            });
          }
        }
      ...
      const uploads = await Promise.all(
        files.map(async (f: any) => {
          const mediaId = crypto.randomUUID();
          const contentType = contentTypeFor(f?.contentType, f?.filename);
          const { presignedUrl, finalUrl } = await generateBrandMediaUploadUrl(
            req.userId!,
            guideline.id,
            mediaId,
            contentType
      ...
      res.json({
        ...
        next: `PUT each file to its presignedUrl with the matching Content-Type, then POST /api/brand-guidelines/${guideline.id}/media/commit with the mediaIds.`,
      });
    } catch (error: any) {
      console.error('[Brand Media Upload URLs] Error:', error);
      res.status(500).json({ error: 'Failed to mint upload URLs', message: error.message });
    }
  }
);

// POST /api/brand-guidelines/:id/media/commit — register assets already in R2
//
// Fingerprinting happens HERE, from the bytes in R2, so the direct-upload path
// keeps the same dedup (across media AND logos) and the same completeness
// recompute as the base64 path. A faster upload route that quietly loses those
// would just move the problem.
router.post('/:id/media/commit', apiRateLimiter, authenticate, async (req: AuthRequest, res) => {
  try {
    if (!req.userId) return res.status(401).json({ error: 'Unauthorized' });
    ...
    if (items.length > MAX_MEDIA_BATCH) {
      return res.status(400).json({ error: `Too many items (max ${MAX_MEDIA_BATCH})` });
    }

    const existingMedia = (guideline.media as unknown as BrandGuidelineMedia[] | null) || [];
    const added: BrandGuidelineMedia[] = [];
    const duplicates: Array<{ mediaId: string; existing: any }> = [];
    const failed: Array<{ mediaId: string; reason: string }> = [];

    // Index grows as the batch is processed — a batch can contain its own dupes.
    const index = fingerprintIndex(guideline);
    ...
    for (const item of items) {
      ...
      if (!mediaId || !url) {
        failed.push({ mediaId, reason: 'mediaId and url are required' });
        continue;
      }
      ...
      try {
        ...
        if (!head.ok) {
          failed.push({ mediaId, reason: `Asset not readable at url (${head.status})` });
          continue;
        }
        const bytes = Buffer.from(await head.arrayBuffer());
        if (!bytes.length) {
          failed.push({ mediaId, reason: 'Uploaded object is empty' });
          continue;
        }
        committedBytes += bytes.length;
        fp = await fingerprint(bytes);
      } catch (err: any) {
        failed.push({ mediaId, reason: err?.message || 'Failed to read uploaded object' });
        continue;
      }

      const dup = findDuplicate(fp, index);
      if (dup?.kind === 'exact') {
        duplicates.push({ mediaId, existing: dup.asset });
        continue;
      }

      const entry: BrandGuidelineMedia = {
        id: mediaId,
        url,
        ...
        hash: fp.sha256,
        size: fp.size,
        ...(fp.phash ? { phash: fp.phash } : {}),
      };
      added.push(entry);
      index.push({ id: entry.id, label: entry.label, hash: entry.hash, phash: entry.phash });
    }

    // Real-size quota check — the mint-time check trusted the client's numbers.
    if (committedBytes > 0) {
      const user = await prisma.user.findUnique({
        where: { id: req.userId },
        select: { subscriptionTier: true, isAdmin: true },
      ...
      if (!user?.isAdmin) {
        ...
        if (!check.allowed) {
          return res.status(413).json({
            error: 'storage_limit_exceeded',
            message: 'Committing these assets would exceed the storage limit for your plan.',
            ...check,
          });
        }
      }
    ...
    if (added.length) {
      allMedia = [...existingMedia, ...added];
      await prisma.brandGuideline.update({
        where: { id: guideline.id },
        data: {
          media: allMedia as any,
          extraction: recomputeExtraction({ ...(guideline as any), media: allMedia }) as any,
        },
      });
      scheduleColorUsageRecompute(guideline.id);
      scheduleAssetAnalysis(guideline.id);
    ...
    // Partial success is the honest answer for a batch: say what landed, what
    // was already there and what broke, instead of failing all fourteen because
    // one PUT never happened.
    res.status(added.length ? 201 : 200).json({
      added,
      duplicates,
      failed,
      allMedia,
      completeness: calculateCompleteness({ ...(guideline as any), media: allMedia }),
    });
  } catch (error: any) {
    console.error('[Brand Media Commit] Error:', error);
    res.status(500).json({ error: 'Failed to commit media', message: error.message });
  }
});
...
router.post('/:id/logos', apiRateLimiter, authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    // Clear space / minimum size / background matrix are arithmetic over the
    // raster we already have in hand — derive them now so the manual is never
    // missing the three pages a designer would otherwise draw by hand.
    // Best-effort: a measurement failure must never cost the user their upload.
    try {
      const bytes = data
        ? Buffer.from(String(data).replace(/^data:[^;]+;base64,/, ''), 'base64')
        : await fetchLogoBytes(logoUrl ?? thumbnailUrl);
      if (bytes?.length) {
        newLogo.rules = await deriveLogoRules(bytes, {
          palette: (guideline.colors as unknown as BrandGuidelineColor[] | null) ?? [],
        });
      }
    } catch (err: any) {
      console.warn('[Brand Logo Upload] rule derivation skipped:', err?.message);
    }
    ...
    const updated = await prisma.brandGuideline.update({
      where: { id: guideline.id },
      data: {
        logos: updatedLogos as any,
        extraction: recomputeExtraction({ ...(guideline as any), logos: updatedLogos }) as any,
      },
    });
...
// POST /api/brand-guidelines/:id/logos/:logoId/rules — derive usage rules
//
// Same derivation the upload path runs, exposed on its own so a logo uploaded
// before this existed can be backfilled, and so a designer can switch the clear
// space module (cap height vs stem) without re-uploading the asset.
router.post(
  '/:id/logos/:logoId/rules',
  apiRateLimiter,
  authenticate,
  async (req: AuthRequest, res) => {
    try {
      ...
      const logos = (guideline.logos as unknown as BrandGuidelineLogo[] | null) || [];
      const target = logos.find((l) => l.id === req.params.logoId);
      if (!target) return res.status(404).json({ error: 'Logo not found' });

      const validModules: ClearSpaceModule[] = ['capHeight', 'halfCapHeight', 'stem'];
      const module = validModules.includes(req.body?.module) ? req.body.module : 'capHeight';
      const safety = Number.isFinite(req.body?.safety)
        ? Math.min(4, Math.max(1, Number(req.body.safety)))
        : 1;
      const persist = req.body?.persist !== false;

      // Prefer the rasterized thumbnail: an SVG or PDF logo has no pixels to
      // measure, and that is exactly what the thumbnail exists for.
      const bytes =
        (await fetchLogoBytes(target.thumbnailUrl)) ?? (await fetchLogoBytes(target.url));
      if (!bytes) {
        return res.status(422).json({
          error: 'Logo bytes unavailable',
          message: 'Não foi possível baixar o arquivo do logo para medir.',
        });
      }

      const rules = await deriveLogoRules(bytes, {
        module,
        safety,
        palette: (guideline.colors as unknown as BrandGuidelineColor[] | null) ?? [],
      });
      ...
      if (persist) {
        const updatedLogos = logos.map((l) => (l.id === target.id ? { ...l, rules } : l));
        await prisma.brandGuideline.update({
          where: { id: guideline.id },
      ...
      res.json({ logoId: target.id, persisted: persist, rules });
    } catch (error: any) {
      console.error('[Brand Logo Rules] Error:', error);
      res.status(500).json({ error: 'Failed to derive logo rules', message: error.message });
    }
  }
);
...
router.post('/:id/health-check', apiRateLimiter, authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    let report;
    try {
      report = await runBrandHealth(guideline as any, { apiKey });
    } catch (err: any) {
      // The credit is charged up front (chargeCredits throws = the gate). An
      // audit that never saw the brand is not a service rendered — refund it,
      // to the same bucket it came from (earned vs monthly).
      if (charge.charged) {
        await refundCreditsWithRetry(req.userId!, 1, charge.deductionSource).catch((e: any) =>
          console.error('[Brand Health Check] refund failed:', e?.message)
        );
      }
      if (err instanceof BrandHealthContextError) {
        console.error('[Brand Health Check] context did not reach the model:', err.message);
        return res.status(503).json({
          error: 'health_context_unavailable',
          message: err.message,
        });
      }
      throw err;
...
import {
  ...
  getFileData,
  parseFigmaUrl,
  extractTextAsMarkdown,
} from '../services/figmaRestApi.js';

// POST /api/brand-guidelines/figma-extract-text — Figma file → markdown
//
// The piece that turns a Figma file into raw material for a guideline shipped
// only as a plugin button that downloaded a file on the user's machine: an agent
// could not reach it and had to ask a person to click. Brand-agnostic on purpose
// — this reads a file, it does not write a guideline, so it can feed
// brand-guidelines-ingest for a brand that does not exist yet.
router.post('/figma-extract-text', apiRateLimiter, authenticate, async (req: AuthRequest, res) => {
  try {
    if (!req.userId) return res.status(401).json({ error: 'Unauthorized' });

    const raw = String(req.body?.fileId || req.body?.fileUrl || '').trim();
    if (!raw) return res.status(400).json({ error: 'fileId or fileUrl is required' });
    const fileKey = raw.startsWith('http') ? parseFigmaUrl(raw)?.fileKey : raw;
    if (!fileKey) return res.status(400).json({ error: 'Could not read a Figma file key' });
    ...
    const result = await extractTextAsMarkdown(fileKey, token);
    res.json(result);
  } catch (error: any) {
    console.error('[Figma Extract Text] Error:', error);
    res.status(500).json({ error: 'Failed to extract text from Figma', message: error.message });
  }
});
...
router.get('/:id/figma-preview', apiRateLimiter, authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    res.json({
      ...
      verdict: figmaSyncVerdict(tokens),
    });
  } catch (error: any) {
