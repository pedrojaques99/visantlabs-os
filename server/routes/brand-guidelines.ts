...
router.put('/:id', apiRateLimiter, authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    if (!parsed.success) {
      return res
        .status(400)
        .json({ error: 'Invalid brand guideline patch', issues: parsed.error.issues });
    }
  ...
  try {
    ...
    switch (source) {
      ...
      case 'pdf':
        ...
        chunks = await parsePdf(
          Buffer.from(data.replace(/^data:application\/pdf;base64,/, ''), 'base64'),
          filename
        );
        break;
      case 'image':
      ...
      case 'fig_file': {
        ...
        const assetEntries = Object.keys(zip.files).filter(
          (n) => !zip.files[n].dir && (n.startsWith('images/') || /\.(png|jpg|jpeg|svg)$/i.test(n))
        );
    ...
    if (guideline) {
      ...
      guideline = await prisma.brandGuideline.update({
        ...
        data: {
          ...
          guidelines: {
            ...((guideline.guidelines as any) || {}),
            ...convertedData.guidelines,
          } as any,
        },
      });
    } else {
      ...
      guideline = await prisma.brandGuideline.create({
        data: {
          ...
          extraction: {
            sources: [
              { type: 'branding_machine', ref: brandingProject.id, date: new Date().toISOString() },
            ],
            completeness: 0,
          } as any,
        },
...
router.get('/:id/compile', apiRateLimiter, authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    res.json({
      outputs: outputs.map((o) => ({ format: o.format, filename: o.filename, content: o.content })),
    });
  } catch (error: any) {
...
router.post('/:id/logos', apiRateLimiter, authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    if (!logoUrl && !thumbnailUrl && !figmaKey && !figmaNodeId) {
      return res
        .status(400)
        .json({ error: 'Provide data, url, thumbnailData, or a figma reference' });
    }
    ...
    res
      .status(201)
      .json({ logo: newLogo, allLogos: updatedLogos, guideline: { ...updated, _id: updated.id } });
  } catch (error: any) {
...
router.delete('/:id/logos/:logoId', apiRateLimiter, authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    if (publicUrl && logoToDelete.url.startsWith(publicUrl)) {
      try {
        await deleteImage(logoToDelete.url);
      } catch (e) {
        /* ignore */
      }
    }
...
Do NOT include fields that already exist.`;
    ...
    if (raw.strategy && typeof raw.strategy === 'object' && !Array.isArray(raw.strategy)) {
      ...
      if (raw.strategy.coreMessage && typeof raw.strategy.coreMessage === 'object')
        s.coreMessage = raw.strategy.coreMessage;
...
[
  {
    ...
    const validated = suggestions
      ...
      .map((s: any) => ({
        ...
        aspectRatio: ['1:1', '16:9', '9:16', '4:3', '4:5'].includes(s.aspectRatio)
          ? s.aspectRatio
          : '1:1',
...
router.get('/public/:slug/context', publicRateLimiter, async (req, res) => {
  try {
    ...
    } = req.query as { format?: string; output?: string; sections?: string };
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const { colors, text, image, checkContrast, checkColors, checkTone } =
        req.body as ComplianceCheckInput;
...
router.put('/:id/figma-link', apiRateLimiter, authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    if (!isValidFigmaUrl(figmaFileUrl)) {
      return res
        .status(400)
        .json({ error: 'Invalid Figma URL. Expected format: figma.com/file/KEY/...' });
    }
  ...
  try {
    ...
    if (!token) {
      return res.status(400).json({
        ...
        message:
          'Configure seu Figma Personal Access Token nas configurações para usar esta funcionalidade.',
        needsToken: true,
      });
    }
...
router.delete(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      await Promise.allSettled((target.vectorIds || []).map((vid) => vectorService.delete(vid)));
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const existing: KnowledgeFile[] =
        (guideline.knowledgeFiles as unknown as KnowledgeFile[] | null) || [];
      const entry = {
        id: uuidv4(),
        fileName: displayName,
        source,
...
router.post('/:id/liveblocks-auth', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const canEdit =
      isOwner ||
      (Array.isArray(guideline.canEdit) && (guideline.canEdit as string[]).includes(req.userId!));
    const canView =
      isOwner ||
      canEdit ||
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const tokens =
        Object.keys(radiusMap).length > 0
      ...
      console.log('[apply-fig-tokens] normalized:', {
        ...
        preClassified: assetClassifications?.length,
        replace,
        source: sourceType,
      });
      ...
      if (images?.length) {
        ...
        if (classifications.length) {
          console.log(
            '[apply-fig-tokens] using pre-computed classifications (skipped Gemini reclassify)'
          );
        } else {
      ...
      try {
        ...
        setImmediate(async () => {
          try {
            ...
            const existingFiles: KnowledgeFile[] =
              (existing!.knowledgeFiles as unknown as KnowledgeFile[] | null) || [];
