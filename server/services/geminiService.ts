...
export const resolveImageBase64 = async (image: UploadedImage): Promise<string> => {
  if (image.base64 && image.base64.length > 0) {
    if (process.env.NODE_ENV === 'development')
      console.log('[dev] resolveImageBase64: using base64, len=', image.base64.length);
    return image.base64;
  }
  ...
  if (image.url) {
    ...
    try {
      const t0 = Date.now();
      if (process.env.NODE_ENV === 'development')
        console.log('[dev] resolveImageBase64: fetch start', image.url);
      ...
      if (process.env.NODE_ENV === 'development')
        console.log(
          '[dev] resolveImageBase64: fetch done',
...
const withRetry = async <T>(apiCall: () => Promise<T>, options: RetryOptions = {}): Promise<T> => {
  const { maxRetries, timeout, onRetry, model = GEMINI_MODELS.IMAGE_FLASH } = options;
...
export const generateMockup = async (
  ...
  return withRetry(
    async () => {
      ...
      if (blockedRating) {
        throw new Error(
          `Generation blocked by safety filters (${blockedRating.category}: ${
            blockedRating.probability
          }). Text: ${textResponse.trim()}`
        );
      }

      throw new Error(
        `No image was generated in the response. Model response: ${
          textResponse.trim() || 'Empty response'
        }`
      );
    },
    {
...
export const analyzeMockupSetup = async (
  ...
  return withRetry(
    async () => {
      ...
      if (process.env.NODE_ENV === 'development')
        console.log(
          '[dev] analyzeMockupSetup: buildAnalysisPrompt done',
          ((Date.now() - t0) / 1000).toFixed(2) + 's'
        );
      ...
      if (process.env.NODE_ENV === 'development')
        console.log(
          '[dev] analyzeMockupSetup: resolveImageBase64 done',
          ((Date.now() - t0) / 1000).toFixed(2) + 's'
        );
      ...
      if (process.env.NODE_ENV === 'development')
        console.log(
          '[dev] analyzeMockupSetup: Gemini generateContent done',
          ((Date.now() - t0) / 1000).toFixed(2) + 's'
        );
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development')
          console.log(
            '[dev] analyzeMockupSetup: validate/parse done',
            ((Date.now() - t0) / 1000).toFixed(2) + 's'
          );
    ...
    async () => {
      ...
      if (params.learnFromHistory !== false) {
        const queryText = [
          ...
          `TAGS: branding=[${harmonized.brandingTags.join(
            ', '
          )}] location=[${harmonized.locationTags.join(
            ', '
          )}] lighting=[${harmonized.lightingTags.join(', ')}]`,
        ]
          .filter(Boolean)
          .join('\n');
        ...
        if (curatedRefs.length > 0) {
          ...
          if (process.env.NODE_ENV !== 'production') {
            console.log(
              `[generateSmartPrompt] RAG injected ${curatedRefs.length} curated references`
            );
          }
        }
      }
      ...
      const promptToGemini = instructionsTemplate
        ...
        .replace(
          '[INSTRUCTIONS]',
          (params.instructions || '') +
            (params.detectedLanguage?.toLowerCase().includes('pt')
              ? ' CRITICAL: The text in the design is in Portuguese (Brazil). Ensure the environment, background elements, and overall composition feel authentic to a Brazilian context and culture. Avoid generic global styles.'
              : '')
        )
      ...
      if (process.env.FEATURE_REFLECTION_LOOP === 'true' && brandBrief && prompt.length > 0) {
        try {
          if (process.env.NODE_ENV !== 'production')
            console.log('[generateSmartPrompt] Launching Reflection Loop...');

          const reflection = await reflectAndRefinePrompt(prompt, brandBrief, params, apiKey);
...
export const changeObjectInMockup = async (
  ...
  return withRetry(
    async () => {
      const prompt = `Keep the same background, environment, lighting, and camera angle, but replace the main object in the image with ${sanitizeForPrompt(
        newObject,
        500
      )}. The new object should be placed in the same position and orientation as the original object, maintaining the same perspective and composition. The environment, background, and all other elements should remain exactly the same.`;
...
${enriched.reusableAssets
  .map((a) => `- "${a.name}" (${a.type}, ${a.width}x${a.height})`)
  .join('\n')}`;
}
...
function buildLinearSection(linearIssue: any): string {
  ...
  const dims =
    linearIssue.formato?.dimensoes?.map((d: any) => `${d.width}x${d.height}`).join(', ') ||
    'não especificado';
  const textos = Object.entries(linearIssue.textos || {})
    .map(([k, v]) => `  - ${k}: "${(v as string).slice(0, 50)}..."`)
    .join('\n');
...
INSTRUCTIONS: Create page "[${linearIssue.identifier}] ${linearIssue.title.slice(
    0,
    50
  )}", one frame per format with dimensions in name, autoPosition: "right", clone assets by sourceName if available, include all specified texts.`;
}
  ...
  return withRetry(
    async () => {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [...history, { parts: [{ text: sanitizedQuery }] }],
        config,
      });
