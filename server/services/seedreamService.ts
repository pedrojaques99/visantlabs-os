...
export async function generateSeedreamImage(
  options: SeedreamGenerateOptions
  ...
  const usedSeed =
    typeof userSeed === 'number' && userSeed >= -1 && userSeed <= 2_147_483_647 ? userSeed : -1; // -1 = random

  // Use user BYOK first, then server key
  const apiKey = specificApiKey || process.env.BYTEPLUS_API_KEY;
  if (!apiKey) {
    throw new Error(
      'No Seedream API key available. Configure BYTEPLUS_API_KEY on the server or provide your own key.'
    );
  }
  ...
  body.guidance_scale =
    typeof guidanceScale === 'number' && guidanceScale >= 1 && guidanceScale <= 10
      ? guidanceScale
      : defaultScale;
  ...
  const data = await meterResult(
    {
      provider: 'seedream',
      model,
      operation: 'seedream-generate',
      resilienceKey: 'seedream',
      apiKeySource: specificApiKey ? 'user' : 'system',
      promptLength: prompt?.length,
      hasInputImage: imageArray.length > 0,
      usage: { images: 1, resolution: resolvedSize },
    },
    withResilience('seedream', async () => {
