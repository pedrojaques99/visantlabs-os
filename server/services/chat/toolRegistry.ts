...
// ── Promote brand admin tools to plugin scope ──
// brand_guideline_update and brand_guideline_create already exist in adminChatTools
// with full schemas and Prisma-direct execution. Promote to 'public' so the plugin
// pre-pass (getChatTools(false)) can use them without duplication.
for (const name of ['brand_guideline_update', 'brand_guideline_create', 'save_to_brand_knowledge']) {
  if (REGISTRY[name]) {
