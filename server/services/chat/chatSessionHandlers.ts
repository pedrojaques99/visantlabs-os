/**
 * Shared chat-session handlers — the agentic chat engine behind both
 * /api/admin-chat (admin "Chat Estratégico") and /api/copilot (Brand Copilot,
 * subscriber-gated). Extracted from routes/adminChat.ts so the two routes are
 * thin wrappers over the SAME sessions, WebSocket and SSE machinery — zero
 * behavioral change for the admin surface.
 */
...
import { executeChatTool } from './toolRegistry.js';
...
// chat session so the user sees real-time feedback without polling.
...
export type ChatPromptVariant = 'admin' | 'copilot';

export interface ChatRouterOptions {
  /** Auth middleware chain (validateAdmin | validateSubscriber). */
...
/**
 * Persona header per surface. The admin variant keeps the internal agency
 * framing; the copilot variant is the sellable product voice — same Visant
 * methodology (it's the differentiator), no admin-internal instructions
 * (reference library curation, user management).
 */
const PROMPT_PERSONA: Record<ChatPromptVariant, string> = {
  admin: `Você é o assistente estratégico de uma agência de branding e design que segue a Metodologia Visant.
...
Seja direto, estratégico e minimalista. Responda no idioma do usuário. Jamais use emojis.`,
  copilot: `Você é o Brand Copilot da Visant — o copiloto criativo do usuário, guiado pela Metodologia Visant.
Especialidades: posicionamento, naming, tom de voz, paletas visuais, referências criativas, mockups, estratégia de marca.
...
export function buildSystemPrompt(
  ...
  brandContext: string,
  ragContext: string,
  variant: ChatPromptVariant = 'admin'
): string {
  const memoryStr =
    [
...
async function executeToolCalls(
  ...
  console.log(`${logPrefix} Executing ${toolCalls.length} tool(s) in parallel`);
...
export function createChatSessionRouter(opts: ChatRouterOptions): express.Router {
  ...
  // Product.metadata — needs a per-user (not per-IP) limiter; keep the existing
  // express-rate-limit infra until then.
  ...
  // GET /sessions — listar sessões (owned + shared com o user)
  router.get('/sessions', guard, async (req: AuthRequest, res: Response) => {
    try {
      await connectToMongoDB();
      const db = getDb();
  ...
  // POST /sessions/:id/share — atualiza share state (owner only)
  router.post('/sessions/:id/share', guard, async (req: AuthRequest, res: Response) => {
    try {
  ...
  router.get('/sessions/:id', guard, async (req: AuthRequest, res: Response) => {
    try {
      const session = await getSession(req.params.id, req.userId!);
      if (!session) return res.status(404).json({ error: 'Sessão não encontrada' });
      res.json({ session });
  ...
  // DELETE /sessions/:id — deletar sessão (owner only)
  router.delete('/sessions/:id', guard, async (req: AuthRequest, res: Response) => {
    try {
      const session = await getSession(req.params.id, req.userId!);
      if (!session) return res.status(404).json({ error: 'Sessão não encontrada' });
      if (sessionOwnerId(session) !== req.userId!) {
  ...
  // PATCH /sessions/:id/brand — travar marca contexto da sessão
  ...
  router.patch('/sessions/:id/brand', guard, async (req: AuthRequest, res: Response) => {
    try {
      const session = await getSession(req.params.id, req.userId!);
      if (!session) return res.status(404).json({ error: 'Sessão não encontrada' });
      if (sessionOwnerId(session) !== req.userId!) {
  ...
  // POST /sessions/:id/upload — ingerir documento na sessão
  router.post('/sessions/:id/upload', guard, async (req: AuthRequest, res: Response) => {
    try {
      const session = await getSession(req.params.id, req.userId!);
      if (!session) return res.status(404).json({ error: 'Sessão não encontrada' });
      ...
      console.error(`${logPrefix} Upload error:`, err);
      res.status(500).json({ error: err.message });
    }
  });

  // POST /sessions/:id/message — enviar mensagem
  router.post('/sessions/:id/message', guard, async (req: AuthRequest, res: Response) => {
    try {
      const session = await getSession(req.params.id, req.userId!);
      if (!session) return res.status(404).json({ error: 'Sessão não encontrada' });
      ...
      // 3. System prompt composto por variante (admin | copilot)
      const systemInstruction =
        buildSystemPrompt(session.memory, brandContext, ragContext, promptVariant) +
        (planMode
      ...
      console.error(`${logPrefix} Message error:`, err);
      res.status(500).json({ error: err.message });
    }
  });

  // POST /sessions/:id/message/stream — SSE streaming
  router.post(
      ...
      try {
        ...
        if (session.messages.length === 0) {
          chatWithLLM(
            ...
            .then((r) => {
              session.title =
                r.text.trim().replace(/['"]/g, '').slice(0, 60) || message.slice(0, 60);
              saveSession(session).catch(() => {});
              broadcastToSession(session._id, {
                type: 'SESSION_TITLE_UPDATED',
                payload: { title: session.title },
  ...
  // POST /sessions/:id/pendings/:pendingId/approve — approve brand knowledge write
  router.post(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        const vectorIds: string[] =
          ingestResult?.ids ?? (ingestResult?.id ? [ingestResult.id] : []);
        ...
        console.error(`${logPrefix} Approve error:`, err);
        res.status(500).json({ error: err.message });
      }
    }
  );

  // POST /sessions/:id/pendings/:pendingId/reject — reject brand knowledge write
  router.post(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        console.error(`${logPrefix} Reject error:`, err);
        res.status(500).json({ error: err.message });
      }
    }
  );
...
// (o /api/copilot/ws aponta para o MESMO room map — sessões são compartilhadas)
