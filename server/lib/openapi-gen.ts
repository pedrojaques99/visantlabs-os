...
export function generateOpenAPISpec(
  ...
  if (!serverUrl.startsWith('http://') && !serverUrl.startsWith('https://')) {
    throw new ValidationError('serverUrl must start with http:// or https://', { serverUrl });
  }
  ...
  try {
    const endpoints: DocEndpoint[] = [
        ...
        parameters: [
          {
            ...
            schema: {
              type: 'string',
              description: 'Public slug of the brand guideline',
              example: 'acme-corp',
            },
          },
        ],
      ...
      },
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            schema: {
              type: 'string',
              description: 'Output format: full (default) or compact (optimized for image gen)',
              example: 'compact',
            },
          },
          {
            ...
            schema: {
              type: 'string',
              description: 'Response type: text (default) or json',
              example: 'json',
            },
          },
        ],
      ...
      // ============ AI / PROMPT TOOLS (7) ============
      {
        ...
        responses: {
          200: {
            ...
            schema: { type: 'object', properties: { improved: { type: 'string' } } },
          },
        },
      },
      {
            ...
            'application/json': {
              schema: {
                ...
                properties: {
                  designType: {
                    type: 'string',
                    description: 'Type of design (e.g. product mockup, social media post)',
                  },
      ...
      },
      {
        ...
        responses: {
          200: {
            description: 'Array of prompt variations',
            schema: {
              type: 'object',
      ...
      },
      {
        ...
        responses: {
          200: {
            description: 'Image description / extracted prompt',
                ...
                colors: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      role: {
                        type: 'string',
                        enum: ['primary', 'secondary', 'accent', 'background', 'neutral'],
                      },
              ...
              schema: {
                ...
                properties: {
                  ...
                  count: {
                    type: 'integer',
                    default: 10,
                    description: 'Number of name suggestions',
                  },
                  style: {
                    type: 'string',
                    description: 'Naming style (invented word, metaphor, compound, real word)',
                  },
                  brandGuidelineId: { type: 'string' },
                },
                required: ['brief'],
...
- 13: Archetypes — brand archetype analysis (Hero, Sage, Lover, Caregiver, etc.)`,
            ...
            'application/json': {
              schema: {
                ...
                properties: {
                  step: {
                    type: 'integer',
                    enum: [1, 5, 6, 7, 8, 9, 10, 11, 12, 13],
                    description: 'Branding step to generate',
                  },
        ...
        responses: {
          200: {
            description: 'Generated branding step result',
              ...
              schema: {
                ...
                properties: {
                  prompts: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'Array of prompts (max 20)',
                  },
                  provider: {
                    type: 'string',
                    enum: IMAGE_PROVIDERS as string[],
    ...
    const spec: OpenAPISpec = {
      ...
      info: {
        ...
        description:
          'REST API for Visant design copilot platform. Supports JWT and API key authentication. AI agents can also connect via MCP (Model Context Protocol) at /api/mcp for direct tool invocation. See /llms.txt for agent discovery.',
        version,
      },
      servers: [
        {
      ...
      components: {
        securitySchemes: {
          ...
          apiKeyAuth: {
            ...
            scheme: 'bearer',
            description:
              'API key authentication for agents. Format: visant_sk_xxxxxxxxxxxx. Get your key from Settings → API Keys.',
          },
        },
      },
      tags: [
        ...
        { name: 'mockups', description: 'Mockup generation' },
        {
          ...
          description: 'Brand guidelines API - public endpoints require no auth',
        },
        { name: 'plugin', description: 'Figma plugin integration' },
        {
          name: 'ai',
          description: 'AI tools — prompt generation, image analysis, color extraction, naming',
        },
        {
          name: 'branding',
          description: 'Brand identity generation — persona, archetype, SWOT, colors, moodboard',
        },
        { name: 'canvas', description: 'Canvas management' },
        { name: 'api-keys', description: 'API key management for agent access' },
        {
          name: 'mcp',
          description: 'MCP (Model Context Protocol) server — connect at /api/mcp via SSE',
        },
      ],
    };
...
export function countEndpoints(spec: OpenAPISpec): number {
  ...
  if (!spec.paths || typeof spec.paths !== 'object') {
    throw new ValidationError('spec.paths is required and must be an object', {
      paths: spec.paths,
    });
  }
