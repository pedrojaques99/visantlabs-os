...
export async function extractPdfStreaming(
  ...
  try {
    ...
    if (
      tokens.isMostlyOutlined &&
      Array.isArray(semantic.fontFamilies) &&
      semantic.fontFamilies.length
    ) {
    ...
    console.warn(
      '[pdf-extract] semantic pass failed (algorithmic data still delivered):',
      err.message
    );
  }
...
function applyColorNames(colors: PdfTokens['colors'], named: Array<{ hex: string; name: string }>) {
