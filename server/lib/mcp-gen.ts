...
const PLATFORM_TOOLS: PlatformToolDef[] = [
  ...
  { name: 'mockup-generate', description: 'Generate a mockup placing a design into a realistic scene. Pass the design as referenceImages URL; describe only the scene in the prompt. See server instructions for workflow.', required: ['prompt'], properties: { prompt: { type: 'string', description: 'Scene/environment ONLY — surface, lighting, camera angle, background.' }, brandGuidelineId: { type: 'string' }, model: { type: 'string', enum: ['gpt-image-2', 'gpt-image-1', 'gemini-3.1-flash-image-preview', 'seedream-3-0'], default: 'gpt-image-2' }, provider: { type: 'string', enum: ['openai', 'gemini', 'seedream'] }, aspectRatio: { type: 'string', enum: ['1:1', '9:16', '16:9', '4:5'] }, resolution: { type: 'string', enum: ['1K', '2K', '4K'] }, referenceImages: { type: 'array', items: { type: 'string' }, description: 'URLs of the design/artwork. Get URLs via upload-image.' } }, cost: 'credits', category: 'mockups', auth: true },
  ...
  // ---- AI ----
  { name: 'upload-image', description: 'Upload a base64 image and get a permanent public URL. Required before mockup-generate/ai-generate-image when you have a local file. Free, no credits, max 20MB.', required: ['data'], properties: { data: { type: 'string', description: 'Base64-encoded image data (with or without data URI prefix).' }, contentType: { type: 'string', enum: ['image/png', 'image/jpeg', 'image/webp', 'image/svg+xml'], default: 'image/png' }, label: { type: 'string', description: 'Optional label for organization.' } }, cost: 'free', category: 'ai', auth: true },
  { name: 'ai-generate-image', description: 'Generate an image from a text prompt. No brand injection, no project saving. For placing existing designs in scenes, use mockup-generate instead. Costs credits.', required: ['prompt'], properties: { prompt: { type: 'string' }, model: { type: 'string', enum: ['gpt-image-2', 'gpt-image-1', 'gemini-3.1-flash-image-preview', 'seedream-3-0'], default: 'gpt-image-2' }, aspectRatio: { type: 'string', enum: ['1:1', '9:16', '16:9', '4:5'], default: '1:1' }, resolution: { type: 'string', enum: ['1K', '2K', '4K'], default: '1K' }, referenceImages: { type: 'array', items: { type: 'string' }, description: 'Reference URLs or base64. Use upload-image for local files.' }, seed: { type: 'integer' } }, cost: 'credits', category: 'ai', auth: true },
  ...
  // ---- Community ----
  { name: 'community-presets', description: 'Browse approved community presets. Filter by category or search by keyword. No auth required.', required: [], properties: { limit: { type: 'integer', default: 20 }, skip: { type: 'integer', default: 0 }, category: { type: 'string' }, search: { type: 'string' } }, cost: 'free', category: 'community', auth: false },
  { name: 'community-preset-get', description: 'Get a single community preset by ID. No auth required.', required: ['id'], properties: { id: { type: 'string' } }, cost: 'free', category: 'community', auth: false },
  { name: 'community-preset-create', description: 'Create a new community preset with prompt, category, and metadata.', required: ['name', 'description', 'prompt', 'category'], properties: { name: { type: 'string' }, description: { type: 'string' }, prompt: { type: 'string' }, category: { type: 'string' }, presetType: { type: 'string' }, aspectRatio: { type: 'string', default: '1:1' }, model: { type: 'string' }, tags: { type: 'array' }, referenceImageUrl: { type: 'string' }, difficulty: { type: 'string' }, context: { type: 'string' }, useCase: { type: 'string' }, examples: { type: 'array' } }, cost: 'free', category: 'community', auth: true },
  { name: 'community-preset-update', description: 'Update your own community preset. Only provided fields are changed.', required: ['id'], properties: { id: { type: 'string' }, name: { type: 'string' }, description: { type: 'string' }, prompt: { type: 'string' }, category: { type: 'string' }, tags: { type: 'array' } }, cost: 'free', category: 'community', auth: true },
  { name: 'community-preset-delete', description: 'Delete your own community preset permanently.', required: ['id'], properties: { id: { type: 'string' } }, cost: 'free', category: 'community', auth: true },
