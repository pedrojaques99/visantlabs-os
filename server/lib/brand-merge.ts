...
export function mergeBrandGuidelines(
  existing: BrandGuideline,
  incoming: Partial<BrandGuideline>
): BrandGuideline {
  ...
  if (incoming.colors?.length) {
    ...
    merged.colors = [
      ...(merged.colors || []),
      ...incoming.colors.filter((c) => !seen.has(c.hex.toUpperCase())),
    ];
  }
  ...
  if (incoming.typography?.length) {
    ...
    merged.typography = [
      ...(merged.typography || []),
      ...incoming.typography.filter((t) => !seen.has(`${t.family}|${t.style || ''}`)),
    ];
  }
  ...
  if (incoming.tokens) {
    ...
    if (incoming.tokens.spacing)
      merged.tokens.spacing = { ...merged.tokens.spacing, ...incoming.tokens.spacing };
    if (incoming.tokens.radius)
      merged.tokens.radius = { ...merged.tokens.radius, ...incoming.tokens.radius };
    if (incoming.tokens.shadows)
      merged.tokens.shadows = { ...merged.tokens.shadows, ...incoming.tokens.shadows };
    if (incoming.tokens.components)
      merged.tokens.components = { ...merged.tokens.components, ...incoming.tokens.components };
  }

  // Guidelines: only fill empty
  if (incoming.guidelines) {
    merged.guidelines = merged.guidelines || {};
    if (!merged.guidelines.voice && incoming.guidelines.voice)
      merged.guidelines.voice = incoming.guidelines.voice;
    if (!merged.guidelines.dos?.length && incoming.guidelines.dos?.length)
      merged.guidelines.dos = incoming.guidelines.dos;
    if (!merged.guidelines.donts?.length && incoming.guidelines.donts?.length)
      merged.guidelines.donts = incoming.guidelines.donts;
    if (!merged.guidelines.imagery && incoming.guidelines.imagery)
      merged.guidelines.imagery = incoming.guidelines.imagery;
  }

  // Strategy: only fill empty
  if (incoming.strategy) {
    merged.strategy = merged.strategy || {};
    if (!merged.strategy.coreMessage?.product && incoming.strategy.coreMessage)
      merged.strategy.coreMessage = incoming.strategy.coreMessage;
    if (!merged.strategy.pillars?.length && incoming.strategy.pillars?.length)
      merged.strategy.pillars = incoming.strategy.pillars;
    if (!merged.strategy.manifesto && incoming.strategy.manifesto)
      merged.strategy.manifesto = incoming.strategy.manifesto;
    if (!merged.strategy.positioning?.length && incoming.strategy.positioning?.length)
      merged.strategy.positioning = incoming.strategy.positioning;
    if (!merged.strategy.archetypes?.length && incoming.strategy.archetypes?.length)
      merged.strategy.archetypes = incoming.strategy.archetypes;
    if (!merged.strategy.personas?.length && incoming.strategy.personas?.length)
    ...
    if (!merged.strategy.graphicSystem && incoming.strategy.graphicSystem)
      merged.strategy.graphicSystem = incoming.strategy.graphicSystem;
  }
  ...
  if ((incoming as any).borders?.length) {
    const seen = new Set(
      (merged as any).borders?.map((b: any) => b.css || `${b.width}:${b.color}`) || []
    );
