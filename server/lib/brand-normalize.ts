...
export interface RawAssetClass {
  index: number;
  category: string;
  logoVariant?: string;
  label?: string;
...
function inferTypographyRole(
  provided: string | undefined,
  size: number | undefined
): TypographyRole {
  if (provided && (TYPOGRAPHY_ROLES as readonly string[]).includes(provided))
    return provided as TypographyRole;
...
const SHADOW_PARTS_RE =
  /(-?\d+(?:\.\d+)?)\s*px\s+(-?\d+(?:\.\d+)?)\s*px\s+(\d+(?:\.\d+)?)\s*px(?:\s+(-?\d+(?:\.\d+)?)\s*px)?\s+(rgba?\([^)]+\)|#[0-9A-Fa-f]{3,8})/;
...
export function normalizeAssetClassifications(input: RawAssetClass[] | undefined) {
  ...
  return input
    .filter(
      (c) =>
        typeof c.index === 'number' && (ASSET_CATEGORIES as readonly string[]).includes(c.category)
    )
    .map((c) => ({
      index: c.index,
      category: c.category as (typeof ASSET_CATEGORIES)[number],
      logoVariant: (LOGO_VARIANTS as readonly string[]).includes(c.logoVariant ?? '')
        ? (c.logoVariant as (typeof LOGO_VARIANTS)[number])
        : undefined,
...
export function normalizeSourceType(input: unknown): ExtractionSource {
  return (VALID_SOURCES as readonly string[]).includes(String(input))
    ? (input as ExtractionSource)
    : 'manual';
}
