...
function parseKiwi(canvasAfterMagic: Uint8Array): any {
  ...
  const decodedSchema = kiwi.decodeBinarySchema(
    new kiwi.ByteBuffer(schemaBuf as unknown as Uint8Array)
  );
...
async function _extractFigTokensImpl(
  buffer: Buffer,
  ...
  for (const node of nodes) {
    ...
    if (!colorHexSeen.has(hex)) {
      colorHexSeen.add(hex);
      colors.push({ hex, name: node.name || '' });
    }
  }
  if (colors.length === 0) {
    ...
    for (const node of nodes) {
      for (const paint of node.fillPaints || []) {
        ...
        const e = cnt.get(hex);
        if (e) e.n++;
        else cnt.set(hex, { hex, name: '', n: 1 });
      }
    }
    colors.push(
      ...Array.from(cnt.values())
        .sort((a, b) => b.n - a.n)
        .slice(0, 20)
    ...
    if (!ex || size > ex.size) {
      fontFamilyMap.set(key, {
        ...
        lineHeight: (() => {
          ...
          if (!lh || lh.value == null) return undefined;
          switch (lh.units) {
            case 'RAW':
              return Math.round(lh.value * 100) / 100;
            case 'PERCENT':
        ...
        letterSpacing: (() => {
          ...
          if (!ls || ls.value == null || Math.abs(ls.value) < 0.5) return undefined;
  ...
  for (const node of nodes) {
    ...
    borderSeen.add(key);
    borders.push({
      name: node.name || `Border ${borders.length + 1}`,
      width,
      color: hex,
      style: 'solid',
    });
  ...
  const radii = Array.from(radiiRaw)
    .sort((a, b) => a - b)
    .slice(0, 8);
  ...
  for (const node of nodes) {
    ...
    const css = `${Math.round(eff.offset?.x ?? 0)}px ${Math.round(
      eff.offset?.y ?? 4
    )}px ${Math.round(eff.radius ?? 12)}px ${Math.round(eff.spread ?? 0)}px rgba(${Math.round(
      c.r * 255
    )},${Math.round(c.g * 255)},${Math.round(c.b * 255)},${parseFloat((c.a ?? 0.15).toFixed(2))})`;
  ...
  for (const node of nodes) {
    const paint = node.fillPaints?.find(
      (p: any) => GRADIENT_TYPES.has(p.type) && p.stops?.length >= 2
    );
    if (!paint) continue;
    const stops = (paint.stops as any[]).map((s: any) => ({
      hex: toHex(s.color).toUpperCase(),
  ...
  const brandTexts = nodes
    ...
    .map((n: any) =>
      (n.textData?.characters || n.name || '').replace(/\n/g, ' ').replace(/\s+/g, ' ').trim()
    )
    .filter((t: string) => t.length >= 20 && t.length <= 300 && !SKIP.some((p) => p.test(t)))
    .filter((t: string, i: number, arr: string[]) => arr.indexOf(t) === i);
  const manifestoCandidates = brandTexts
    .filter(
      (t: string) =>
        t.length > 40 &&
        /cuidar|valor|missão|visão|acredit|transform|result|solução|inovação|pele|beleza|eficácia|intelig/i.test(
          t
        )
    )
    .sort((a: string, b: string) => b.length - a.length);
  const taglineCandidates = brandTexts
    .filter(
      (t: string) =>
        t.length >= 20 && t.length <= 80 && /mizz|pele|cuidado|rotina|result|transfor/i.test(t)
    )
    .sort((a: string, b: string) => a.length - b.length);
  const claims = brandTexts
    .filter(
      (t: string) =>
        t.length >= 15 && t.length <= 60 && /testado|livre de|sem|natural|eficá|derma|gine/i.test(t)
    )
    .slice(0, 5);
  const strategy: NonNullable<FigExtractResult['strategy']> = {};
  if (manifestoCandidates[0]) strategy.manifesto = manifestoCandidates[0];
  if (taglineCandidates[0] && taglineCandidates[0] !== manifestoCandidates[0])
    strategy.tagline = taglineCandidates[0];
  if (claims.length) strategy.claims = claims;
  const descCandidate = manifestoCandidates.find(
    (t: string) => t !== strategy.manifesto && t.length > 30
  );
  ...
  if (zip) {
    ...
    const assetPaths = Object.keys(zip.files).filter(
      (n) => !zip!.files[n].dir && n.startsWith('images/') && n !== 'thumbnail.png'
    );
    let taken = 0;
    for (const path of assetPaths) {
      ...
      const compressedSize: number =
        (zip.files[path] as any)._data?.compressedSize ??
