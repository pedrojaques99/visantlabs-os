...
export const CACHE_TTL = {
  ...
  ADMIN_ANALYTICS: 5 * 60, // 5min — admin product analytics aggregates
} as const;
...
export const CacheKey = {
  ...
  mockupGen: (userId: string, promptHash: string) => `mockup:${userId}:${promptHash}`,

  pluginContext: (fileId: string, brandId: string, hash: string) =>
    `plugin:ctx:${fileId}:${brandId}:${hash}`,

  brandContext: (guidelineId: string, format: string) => `context:${guidelineId}:${format}`,

  presetSearch: (category: string, queryHash: string, page: number) =>
    `preset:${category}:${queryHash}:p${page}`,

  sharePublic: (shareId: string) => `share:${shareId}`,

  // P1: AI endpoints
  aiText: (op: string, userId: string, hash: string) => `ai:${op}:${userId}:${hash}`,

  // P1: Brand intelligence
  brandIntel: (guidelineId: string) => `brand-intel:${guidelineId}`,
  ...
  // P2: Video generation
  videoGen: (userId: string, hash: string) => `video:${userId}:${hash}`,

  // P2: Image search/extraction
  imageSearch: (userId: string, hash: string) => `images:search:${userId}:${hash}`,

  imageExtract: (userId: string, hash: string) => `images:extract:${userId}:${hash}`,

  instagramExtract: (userId: string, hash: string) => `images:instagram:${userId}:${hash}`,

  // P2: Figma
  figmaGen: (userId: string, hash: string) => `figma:gen:${userId}:${hash}`,

  // Admin chat sessions
  adminChatSession: (sessionId: string) => `admin-chat:session:${sessionId}`,

  // Figma operation queue (pending ops when plugin is offline)
  figmaOpQueue: (fileId: string) => `figma:queue:${fileId}`,

  // Asset pipeline (cross-tool "Send to →" queue per user)
  assetPipeline: (userId: string) => `pipeline:${userId}`,

  // Visual search
  visualSearch: (hash: string) => `vsearch:${hash}`,

  letterCrop: (letter: string, sourceId: string) => `lcrop:${letter}:${sourceId}`,

  // Admin product analytics (windowed aggregates)
  adminAnalytics: (days: number) => `admin:analytics:${days}`,
} as const;

export const CacheInvalidation = {
  onBrandEdit: (guidelineId: string, pattern = '*') => [
    `context:${guidelineId}:${pattern}`,
    `plugin:ctx:*:${guidelineId}:${pattern}`,
    `brand-intel:${guidelineId}`,
  ],

  onTemplateChange: () => [`plugin:ctx:*:*:*`],

  onPresetChange: (category: string) => [`preset:${category}:*`],
