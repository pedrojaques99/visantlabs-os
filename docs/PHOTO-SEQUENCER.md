...
> O comentário no topo de `slideshowService.ts` explica por que o concat _demuxer_ não serve (perde frames com still images). Não reescrever sem ler.
...
- **Toda task trata o próprio erro.** `OpButton` chama `runner.run()` sem `.catch`, e `useOpRunner` não tem `catch` — só `finally`. Uma task que rejeita vira _unhandled rejection_: o spinner para e o usuário não vê erro. Todas as tasks deste painel usam try/catch + `showToast`. **Vale pra qualquer painel novo.**
