...
WEIGHT_ORDER.forEach((w, i) => {
  WEIGHT_RANK[w.toLowerCase()] = i;
});
...
function classifyTextRole(node: TextNode): string {
  ...
  if (name.includes('caption') || name.includes('label') || name.includes('footnote'))
    return 'caption';
...
async function resolveFamily(
...
): Promise<{ family: string; styles: string[] } | null> {
  ...
  for (const f of fonts) {
    ...
    if (!familyMap.has(actual)) {
      familyMap.set(actual, new Set());
      familyNormalized.set(norm, actual);
    }
    familyMap.get(actual)!.add(f.fontName.style);
  }
  for (const [norm, actual] of familyNormalized) {
    if (norm === needle) return { family: actual, styles: [...familyMap.get(actual)!] };
  }
  for (const [norm, actual] of familyNormalized) {
    if (norm.startsWith(needle) || needle.startsWith(norm))
      return { family: actual, styles: [...familyMap.get(actual)!] };
  }
  for (const [norm, actual] of familyNormalized) {
...
export async function applyBrandGuidelinesLocally(config: any) {
  ...
  if (!selection || selection.length === 0) {
    postToUI({
      type: 'ERROR',
      message: 'Selecione pelo menos um frame ou elemento para aplicar a brand.',
    });
    return;
  }
  ...
  const bodyEntry =
    fontsWithValue.find((t: any) => t.id === 'secondary') || fontsWithValue[1] || fontsWithValue[0];

  const resolvedHeading = headingEntry?.value
    ? await resolveFamily(
        headingEntry.value.family || headingEntry.value.name || headingEntry.value
      )
    : null;
  const resolvedBody = bodyEntry?.value
    ? await resolveFamily(bodyEntry.value.family || bodyEntry.value.name || bodyEntry.value)
    : null;

  const brandColors = config.colors ? Object.values(config.colors) : [];
  const getRoleHex = (role: string) =>
    (brandColors.find((c: any) => c.role === role) as any)?.value;
  ...
  const pickColor = (
    ...
    b: number,
    isText: boolean,
    isLargeShape: boolean,
    nodeName: string
  ): string | null => {
    const lum = getLuminance(r, g, b);
    const lowerName = nodeName.toLowerCase();
    const isAccent =
      lowerName.includes('accent') || lowerName.includes('star') || lowerName.includes('highlight');
  ...
  async function traverse(node: SceneNode) {
    ...
    if (node.type === 'TEXT') {
      ...
      try {
        ...
        const resolved = isHeading
          ? resolvedHeading || resolvedBody
          : resolvedBody || resolvedHeading;
        ...
        if (resolved) {
          ...
          try {
            ...
            try {
              ...
              fontsFailed.push(`${resolved.family} "${targetStyle}" @ "${node.name}"`);
            }
          }
        }
      } catch (err) {
        console.warn(`[Apply Brand] Font error on "${node.name}":`, err);
      }
    }
    ...
    if ('fills' in node) {
      ...
      if (Array.isArray(fills) && fills.length > 0) {
        ...
        const isLargeShape =
          ...
          Math.max(node.width, node.height) > 400;
        const newFills = fills.map((fill) => {
          if (fill.type === 'SOLID') {
            const chosenHex = pickColor(
              fill.color.r,
              fill.color.g,
              fill.color.b,
              isText,
            ...
            if (chosenHex) {
              colorsApplied++;
              return { ...fill, color: hexToRgb(chosenHex) };
            }
          }
            ...
            if (gradientFill.gradientStops?.length) {
              const newStops = gradientFill.gradientStops.map((stop: ColorStop) => {
                ...
                if (chosenHex) {
                  ...
                  return { ...stop, color: { ...stop.color, r: rgb.r, g: rgb.g, b: rgb.b } };
                }
                return stop;
              });
              gradientsApplied++;
              return { ...gradientFill, gradientStops: newStops };
            }
          }
          return fill;
        });
  ...
  const debugReport = {
    input: {
      typographySlots: (config.typography || []).map((t: any) => ({
        id: t.id,
        label: t.label,
        value: t.value,
      })),
      colorsCount: brandColors.length,
    },
    resolved: {
  ...
  if (!resolvedHeading && headingEntry?.value) {
    violations.push({
      node: '<config>',
      rule: 'heading-font-unresolved',
      detail: String(headingEntry.value.family || headingEntry.value.name || headingEntry.value),
    });
  }
  if (!resolvedBody && bodyEntry?.value) {
    violations.push({
      node: '<config>',
      rule: 'body-font-unresolved',
      detail: String(bodyEntry.value.family || bodyEntry.value.name || bodyEntry.value),
    });
  }
  ...
  const telemetry = {
    ...
    stats: {
      ...
      fontsApplied,
      fontsFailedCount: fontsFailed.length,
      colorsApplied,
      gradientsApplied,
      textNodesSkipped,
      roleCounts,
  ...
  const summary =
    `Brand aplicada! ${fontsApplied} fonte(s), ${colorsApplied} cor(es), ${gradientsApplied} gradiente(s).` +
    (fontsFailed.length > 0 ? ` ⚠️ ${fontsFailed.length} falhas.` : '');
