...
export function usePluginMessages() {
  ...
  const applyOperationsFromAPI = useCallback(
    (operations: any[]) => {
      send({
  ...
  const exportNodeImage = useCallback(
    (nodeId: string, format: 'SVG' | 'PNG' = 'PNG') => {
      send({
  ...
  const pasteGeneratedImage = useCallback(
    (imageData: string, prompt: string, width?: number, height?: number, isUrl?: boolean) => {
      send({
  ...
  const scaffoldAgentLibrary = useCallback(
    (brand: any) => {
      send({
  ...
  const saveDesignSystem = useCallback(
    (designSystem: any) => {
      send({
  ...
  const saveBrandGuideline = useCallback(
    (selectedId: string, guideline: any) => {
      send({
  ...
  const linkGuideline = useCallback(
    (guidelineId: string, autoLoad?: boolean) => {
      send({
        type: 'LINK_GUIDELINE',
        guidelineId,
        autoLoad,
  ...
  const saveLocalBrandConfig = useCallback(
    (config: any) => {
      send({
  ...
  const applyBrandGuidelines = useCallback(
    (brand: any) => {
      send({
  ...
  const createStickyPrompt = useCallback(
  ...
  const varySelectionColors = useCallback(
    (brandColors?: string[]) => {
      send({
  ...
  const brandLintFocus = useCallback(
    (nodeId: string) => {
      send({
  ...
  const fixBrandIssues = useCallback(
    (brand?: any) => {
      send({
        type: 'BRAND_LINT_FIX',
        brand,
  ...
  const responsiveMultiply = useCallback(
