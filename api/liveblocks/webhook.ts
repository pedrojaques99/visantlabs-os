...
const verifyWebhookSignature = (payload: string, signature: string, secret: string): boolean => {
  if (!secret) {
    if (isDev)
      console.warn('⚠️ LIVEBLOCKS_WEBHOOK_SECRET not configured - skipping signature verification');
...
const processWebhookEvent = async (event: LiveblocksWebhookEvent): Promise<void> => {
  try {
    ...
    if (isDev)
      console.log('🔵 Processing Liveblocks webhook event:', {
        type: event.type,
        roomId: event.roomId,
        userId: event.userId,
    ...
    switch (event.type) {
      ...
      case 'userEntered':
        if (isDev)
          console.log('👤 User entered room:', { roomId: event.roomId, userId: event.userId });
        // Could update presence or send notifications
        break;

      case 'userLeft':
        if (isDev)
          console.log('👋 User left room:', { roomId: event.roomId, userId: event.userId });
      ...
      case 'commentCreated':
        if (isDev)
          console.log('💬 Comment created:', { roomId: event.roomId, userId: event.userId });
        // Could send notifications to other users
        break;

      case 'commentEdited':
        if (isDev)
          console.log('✏️ Comment edited:', { roomId: event.roomId, userId: event.userId });
        break;

      case 'commentDeleted':
        if (isDev)
          console.log('🗑️ Comment deleted:', { roomId: event.roomId, userId: event.userId });
        break;

      case 'commentReactionAdded':
        if (isDev)
          console.log('👍 Comment reaction added:', { roomId: event.roomId, userId: event.userId });
        break;

      case 'commentReactionRemoved':
        if (isDev)
          console.log('👎 Comment reaction removed:', {
            roomId: event.roomId,
            userId: event.userId,
          });
        break;
      ...
      case 'threadCreated':
        if (isDev)
          console.log('🧵 Thread created:', { roomId: event.roomId, userId: event.userId });
        break;

      case 'threadDeleted':
        if (isDev)
          console.log('🗑️ Thread deleted:', { roomId: event.roomId, userId: event.userId });
        break;

      case 'threadMarkedAsResolved':
        if (isDev)
          console.log('✅ Thread marked as resolved:', {
            roomId: event.roomId,
            userId: event.userId,
          });
        break;
      ...
      case 'threadMarkedAsUnresolved':
        if (isDev)
          console.log('❌ Thread marked as unresolved:', {
            roomId: event.roomId,
            userId: event.userId,
          });
        break;
      ...
      case 'notification':
        if (isDev)
          console.log('🔔 Notification event:', { roomId: event.roomId, userId: event.userId });
...
export default async (req: any, res: any) => {
  if (isDev)
    console.log('🔵 Liveblocks webhook endpoint hit:', {
      method: req.method,
      url: req.url,
      headers: {
