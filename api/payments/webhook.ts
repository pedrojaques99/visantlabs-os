    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (session.mode === 'subscription') {
          ...
          const customerId =
            typeof customerIdRaw === 'string' ? customerIdRaw : (customerIdRaw as any)?.id || null;
          ...
          if (isDev)
            console.log('✅ Checkout session completed for subscription:', {
              subscriptionId,
              customerId,
              customerEmail,
            });
          ...
          try {
            ...
            try {
              ...
              if (subError.code === 'resource_missing') {
                if (isDev)
                  console.warn(
                    '⚠️ Subscription not found immediately after checkout. This can happen if the event fires before subscription is fully created.'
                  );
                ...
                if (isDev)
                  console.log(
                    '💡 Using default subscription values. Subscription will be updated when customer.subscription.updated event fires.'
                  );
              } else {
            ...
            if (subscription) {
              ...
              if (isDev)
                console.log(
                  '⚠️ Using default subscription period (30 days) - will be updated by subscription.updated event'
                );
            }
            ...
            if (!user && customerEmail) {
              ...
              if (user) {
                await db
                  .collection('users')
                  .updateOne({ _id: user._id }, { $set: { stripeCustomerId: customerId } });
              }
            }
          ...
          try {
            ...
            try {
              ...
              if (isDev)
                console.log('📦 Line items retrieved:', {
              ...
              if (isDev)
                console.warn(
                  '⚠️ Could not retrieve line items, will use amount fallback:',
              ...
              if (lineItemsError.message?.includes('No such checkout session')) {
                if (isDev)
                  console.log(
                    '⚠️ Checkout session not found - this may happen when re-sending webhooks manually or with Payment Links.'
                  );
                if (isDev)
                  console.log(
                    '💡 Will use amount fallback based on session.amount_total from event data.'
                  );
              }
              ...
              credits = 0;
              if (isDev)
                console.log('🔄 Credits reset to 0, proceeding with amount-based fallback...');
            }
            ...
            if (!credits || credits <= 0) {
              ...
              if (isDev)
                console.log('📊 Session data available:', {
                  amountTotal,
                  amountTotalFormatted: amountTotal
                    ? `R$ ${(amountTotal / 100).toFixed(2)}`
                    : 'N/A',
                  amountSubtotal: session.amount_subtotal,
                  amountSubtotalFormatted: session.amount_subtotal
                    ? `R$ ${(session.amount_subtotal / 100).toFixed(2)}`
                    : 'N/A',
                });

              const normalizedCurrency = session.currency
                ? session.currency.toUpperCase()
                : undefined;
              const amountForMatching =
                session.amount_subtotal && session.amount_subtotal > 0
              ...
              const hasCoupon =
                ...
                session.amount_subtotal !== amountTotal;
              if (hasCoupon) {
                const discountAmount = session.amount_subtotal - amountTotal;
                const discountPercent = ((discountAmount / session.amount_subtotal) * 100).toFixed(
                  2
                );
                if (isDev)
              ...
              if (isDev)
                console.log('🔍 Calling getCreditsByAmount:', {
                  amountForMatching,
              ...
              if (credits > 0) {
                ...
                if (isExactMatch) {
                  if (isDev)
                    console.log('✅ Matched credits by amount (exact match):', {
                  ...
                  if (isDev)
                    console.log('✅ Matched credits by amount (with coupon/tolerance):', {
                      amountUsed: amountForMatching,
                      amountFormatted: `R$ ${(amountForMatching / 100).toFixed(2)}`,
                      expectedPrice: `R$ ${expectedPrice.toFixed(2)}`,
                      discount: `R$ ${(
                        (expectedPriceInMinorUnits - amountForMatching) /
                        100
                      ).toFixed(2)}`,
                      discountPercent: `${(
                        ((expectedPriceInMinorUnits - amountForMatching) /
                          expectedPriceInMinorUnits) *
                        100
                      ).toFixed(2)}%`,
                      amountTotal,
                      amountSubtotal: session.amount_subtotal,
                ...
                console.error(
                  '❌ Could not determine credits - amount does not match any known packages:',
                  {
                    productId,
                    amountTotal,
                    amountSubtotal: session.amount_subtotal,
              ...
              if (isDev)
                console.log(
                  '✅ Using credits from product metadata (fallback not needed):',
                  credits
                );
            }
            ...
            if (!user && session.client_reference_id) {
              if (isDev)
                console.log(
                  '🔍 Step 2: Looking up user by client_reference_id:',
                  session.client_reference_id
                );
              try {
                user = await db
                  .collection('users')
                  .findOne({ _id: new ObjectId(session.client_reference_id) });
                if (isDev)
                  console.log('🔍 User lookup by client_reference_id:', {
                ...
                if (user) {
                  if (isDev)
                    console.log('✅ User found by client_reference_id:', {
                      userId: user._id,
                      email: user.email,
                    });
            ...
            if (!user && session.metadata?.userId) {
              if (isDev)
                console.log(
                  '🔍 Step 3: Looking up user by metadata.userId:',
                  session.metadata.userId
                );
              try {
                user = await db
                  .collection('users')
                  .findOne({ _id: new ObjectId(session.metadata.userId) });
                if (isDev)
                  console.log('🔍 User lookup by metadata.userId:', {
                ...
                if (user) {
                  if (isDev)
                    console.log('✅ User found by metadata.userId:', {
                      userId: user._id,
                      email: user.email,
                    });
              ...
              if (user) {
                ...
                if (!customerId && session.payment_intent) {
                  try {
                    ...
                    if (paymentIntentId) {
                      ...
                      if (paymentIntent.customer) {
                        ...
                        if (piCustomerId) {
                          ...
                          if (isDev)
                            console.log('✅ Found customerId from payment intent:', piCustomerId);
                        }
                      }
                    }
                  } catch (piError: any) {
                    if (isDev)
                      console.warn(
                        '⚠️ Could not retrieve payment intent for customerId:',
                        piError.message
                      );
                  }
                }
            ...
            if (!finalCustomerId && session.payment_intent) {
              if (isDev)
                console.log('🔍 Step 5: Attempting to get customerId from payment intent...');
              try {
                const paymentIntentId =
                  typeof session.payment_intent === 'string'
                    ? session.payment_intent
                  ...
                  if (paymentIntent.customer) {
                    ...
                    if (piCustomerId) {
                      ...
                      if (!user) {
                        if (isDev)
                          console.log(
                            '🔍 Looking up user by payment intent customerId:',
                            piCustomerId
                          );
                        user = await db
                          .collection('users')
                          .findOne({ stripeCustomerId: piCustomerId });
                        if (isDev)
                          console.log('🔍 User lookup by payment intent customer:', {
                            customerId: piCustomerId,
                            found: !!user,
                          });
                        if (user) {
                          if (isDev)
                            console.log('✅ User found by payment intent customer:', {
                              userId: user._id,
                              email: user.email,
                            });
              ...
              try {
                ...
                if (!user.stripeCustomerId) {
                  if (finalCustomerId) {
                    ...
                    if (isDev)
                      console.log('🔗 Associating existing Stripe customer with user account...');

                    // Verify customer exists in Stripe
                    try {
                      await stripe.customers.retrieve(finalCustomerId);
                      if (isDev)
                        console.log('✅ Verified Stripe customer exists:', finalCustomerId);
                      updateFields.$set = { stripeCustomerId: finalCustomerId };
                      if (isDev)
                        console.log('💾 Will update stripeCustomerId to:', finalCustomerId);
                    } catch (error: any) {
                      if (error.code === 'resource_missing') {
                        // Customer doesn't exist, create one
                        if (isDev)
                          console.log(
                            '⚠️ Customer ID not found in Stripe, creating new customer...'
                          );
                    ...
                    if (isDev)
                      console.log(
                        '📝 Creating new Stripe customer for user (no customerId found)...'
                      );
                    const stripeCustomer = await stripe.customers.create({
                      email: customerEmail || user.email,
                      metadata: {
                  ...
                  if (isDev)
                    console.log(
                      '🔄 Updating stripeCustomerId from',
                ...
                const updateResult = await db
                  .collection('users')
                  .updateOne({ _id: user._id }, updateFields);
      ...
      case 'invoice.payment_failed': {
        ...
        if (isDev)
          console.log(`📥 Received ${event.type} event (not processed in dedicated handler)`);
...
export default async (req: any, res: any) => {
  ...
  if (abacateSecret && !sig) {
    ...
    if (webhookSecretQuery && !webhookSecretHeader) {
      console.warn(
        '⚠️ DEPRECATED: AbacatePay webhook secret in URL query param. Use X-AbacatePay-Secret header instead.'
      );
    }
    ...
    try {
      ...
      try {
        ...
        console.error('❌ Failed to parse AbacatePay webhook body:', parseError.message);
      ...
      // Call abacatepay service to process webhook
      const result = await abacatepayService.processWebhook(body, db);

      if (result.success) {
        if (isDev) console.log('✅ AbacatePay webhook processed successfully:', result.message);
        return res.status(200).json({ success: true, message: result.message });
      } else {
        if (isDev) console.error('❌ AbacatePay webhook processing failed:', result.message);
  ...
  try {
    // Read raw body - essential for signature verification!
    if (Buffer.isBuffer(req.body)) {
      bodyBuffer = req.body;
    } else if (typeof req.body === 'string') {
      bodyBuffer = Buffer.from(req.body, 'utf8');
    } else {
      // Re-read from stream if not available in req.body
      if (isDev) console.warn('⚠️ Webhook body not present as Buffer - reading from stream...');
      const chunks: Buffer[] = [];
      await new Promise<void>((resolve, reject) => {
