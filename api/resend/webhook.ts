...
const verifyWebhookSignature = (payload: string, signature: string, secret: string): boolean => {
  ...
  try {
    ...
    const receivedSignature =
      signatureParts.length > 2 ? signatureParts[2] : signatureParts[signatureParts.length - 1];
...
export default async (req: any, res: any) => {
  if (isDev)
    console.log('📧 Resend webhook endpoint hit:', {
      method: req.method,
      url: req.url,
      headers: {
  ...
  if (RESEND_WEBHOOK_SECRET) {
    ...
    const isValid = verifyWebhookSignature(bodyString, signature as string, RESEND_WEBHOOK_SECRET);
