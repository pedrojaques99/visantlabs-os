...
import { Users, Plus, Search, X } from '@/lib/ui/icons';
...
export const CommunityPresetsSidebar: React.FC<CommunityPresetsSidebarProps> = ({
  ...
  const handleToggleLike = async (presetId: string) => {
    ...
    // Optimistic update
    const toggleLikeInList = (list: CommunityPrompt[]) =>
      list.map((p) => {
        if (p.id === presetId) {
                ...
                onClick={() => {
                  if (onImportPreset) {
                    ...
                    onImportPreset(
                      ...
                      activeTab === 'all' ? preset.category : activeTab
                    );
                    if (onClose) onClose();
                  }
                }}
  ...
  if (variant === 'sheet') {
    return (
      <Sheet open={isOpen} onOpenChange={onClose}>
        <SheetContent
          side="right"
          className="w-full sm:max-w-md p-0 bg-neutral-950 border-neutral-800"
        >
          {content}
        </SheetContent>
      </Sheet>
