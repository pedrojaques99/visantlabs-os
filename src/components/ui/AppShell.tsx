...
export const AppShellTopBar: React.FC<{
  ...
  <div
    className={cn(
      'absolute top-0 left-0 right-0 flex items-center justify-between px-2 sm:px-3 py-2 z-10',
      className
    )}
  >
...
export const AppShellPanel: React.FC<{
  ...
  <div
    ...
    style={{
      width,
      ...(!visible
        ? { transform: `translateX(${side === 'right' ? width + 20 : -(width + 20)}px)` }
        : undefined),
    }}
  >
    {children}
...
export const AppShellStatusBar: React.FC<{ children: React.ReactNode; className?: string }> = ({
  children,
  className,
}) => (
