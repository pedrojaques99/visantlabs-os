...
export const UpscaleBicubicNode: React.FC<NodeProps<Node<UpscaleBicubicNodeData>>> = memo(
  ({ data, selected, id, dragging }) => {
      ...
      try {
        ...
        if (data.addTextNode) {
          ...
          if (currentNode) {
            ...
            toast.success(t('canvasNodes.outputNode.imageDescriptionGenerated'), {
              duration: 2000,
            });
          } else {
      ...
      async (e: React.MouseEvent) => {
        ...
        try {
          ...
          if (
            ...
            imageUrlToDownload.startsWith('https://')
          ) {
    ...
    }, [data.connectedImage, data.onApply, id, isLoading, hasResult]);
        ...
        <div className="p-4 flex flex-col gap-[var(--node-gap)] flex-1 overflow-hidden">
          ...
          {hasResult && !isLoading && (
            <div className="space-y-2 flex-1 min-h-[240px] flex flex-col pt-2">
              {hasVideoResult ? (
                <div className="relative w-full h-full bg-neutral-950/20 rounded-md overflow-hidden border-node border-neutral-700/50 flex-1 min-h-0">
                  <video
        ...
        {!dragging && (resultImageUrl || resultVideoUrl) && (
          <NodeActionBar selected={selected} getZoom={getZoom}>
            ...
            <NodeButton
              ...
              title={
                isDescribing
                  ? t('canvasNodes.imageNode.analyzingImage')
