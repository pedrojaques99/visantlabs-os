...
import { Compass, PanelRight, Sparkles, Image as ImageIcon, Check, Loader2 } from 'lucide-react';
...
export const DirectorNode = memo(({ data, selected, id, dragging }: NodeProps<any>) => {
  ...
  const getStatusColor = () => {
    ...
    if (connectedImage) return 'text-neutral-400';
    ...
    >
      ...
      {generatedPrompt && (
        <div className="mt-3 p-2 rounded-lg border border-green-500/30 bg-green-500/5">
          ...
          <p className="text-xs text-neutral-300 line-clamp-3">{generatedPrompt}</p>
        </div>
      )}
    </NodeContainer>
  );
