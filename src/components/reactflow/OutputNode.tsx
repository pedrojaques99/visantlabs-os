...
export const OutputNode = memo(
  ({ data, selected, id, dragging }: NodeProps<any>) => {
    ...
    const [savedMockupId, setSavedMockupId] = useState<string | null>(
      nodeData.savedMockupId || null
    );
        ...
        {!dragging && mediaUrl && (
          <NodeActionBar selected={selected} getZoom={getZoom}>
            ...
            <SendToButton
              ...
              imageBase64={
                typeof mediaUrl === 'string' && mediaUrl.startsWith('data:')
                  ? mediaUrl
                  : nodeData.resultImageBase64
              }
              mimeType="image/png"
  ...
  (prevProps, nextProps) => {
    ...
    if (
      prevData.resultImageUrl !== nextData.resultImageUrl ||
