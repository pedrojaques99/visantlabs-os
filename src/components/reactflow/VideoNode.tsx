...
export const VideoNode = memo(
  ({ data, selected, id, dragging }: NodeProps<any>) => {
    ...
    const showSecondHandle =
      mode === GenerationMode.FRAMES_TO_VIDEO || mode === GenerationMode.REFERENCES;
    ...
    useEffect(() => {
      ...
      if (nodeData.resolution && nodeData.resolution !== resolution)
        setResolution(nodeData.resolution);
      if (nodeData.duration && nodeData.duration !== duration) setDuration(nodeData.duration);
      if (nodeData.isLooping !== undefined && nodeData.isLooping !== isLooping)
        setIsLooping(nodeData.isLooping);
      if (nodeData.isBrandActive !== undefined && nodeData.isBrandActive !== isBrandActive)
        setIsBrandActive(nodeData.isBrandActive);
      ...
      if (nodeData.cfgScale !== undefined && nodeData.cfgScale !== cfgScale)
        setCfgScale(nodeData.cfgScale);
    }, [
      nodeData.prompt,
    ...
    const { debouncedUpdate: debouncedUpdateData, immediateUpdate: updateData } =
      useNodeDataUpdater<VideoNodeData>(nodeData.onUpdateData, id);
    ...
    const handleGenerate = useCallback(async () => {
      ...
      const params: GenerateVideoParams = {
        ...
        startFrame:
          mode === GenerationMode.FRAMES_TO_VIDEO ? toInput(nodeData.connectedImage1) : undefined,
        endFrame:
          mode === GenerationMode.FRAMES_TO_VIDEO ? toInput(nodeData.connectedImage2) : undefined,
        inputVideo:
          mode === GenerationMode.EXTEND_VIDEO ? toInput(nodeData.connectedVideo) : undefined,
        referenceImages:
          mode === GenerationMode.REFERENCES
            ? [
                nodeData.connectedImage1,
                nodeData.connectedImage2,
                nodeData.connectedImage3,
    ...
    const isGenerateDisabled =
      isLoading || (!prompt && !connectedText && !nodeData.connectedImage1);
          ...
          <div className="node-margin">
            <NodeMediaDisplay
              url={
                ...
                (nodeData.resultVideoBase64
                  ? nodeData.resultVideoBase64.startsWith('data:')
                    ? nodeData.resultVideoBase64
                    : `data:video/mp4;base64,${nodeData.resultVideoBase64}`
                  : null)
              }
              isVideo={true}
              isLoading={isLoading}
