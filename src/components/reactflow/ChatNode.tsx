...
export const ChatNode = memo(({ data, selected, id, dragging }: NodeProps<any>) => {
  ...
  const hasContext =
    connectedImages.length > 0 || !!nodeData.connectedText || !!nodeData.connectedStrategyData;
    ...
    <NodeContainer selected={isSelected} dragging={isDragging} className="h-full overflow-hidden">
      ...
      {[1, 2, 3, 4].map((idx) => (
        <LabeledHandle
          ...
          position={Position.Left}
          id={`input-${idx}`}
          label={`${t('canvasNodes.chatNode.image')} ${idx}`}
          handleType="image"
          style={{ top: 120 + idx * 60 }}
        />
      ))}
      <Handle
        type="source"
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {nodeData.onOpenSidebar && (
                <NodeButton
                  ...
                  title={t('canvasNodes.chatNode.openAsPanel')}
                >
                  <PanelRight size={14} />
                </NodeButton>
              )}
              <NodeButton
                ...
                className={cn(
                  showSystemPromptEditor && 'text-brand-cyan border-neutral-800 bg-brand-cyan/5'
                )}
              >
                <Settings2 size={14} />
              </NodeButton>
      ...
      {isSelected && !isDragging && (
        <NodeResizer
          ...
          minHeight={600}
          onResize={(_, { width }) =>
            handleResizeWithDebounce(nodeId, width, 'auto', nodeData.onResize)
          }
        />
      )}
    </NodeContainer>
