...
export const PromptNode = memo(
  ({ data, selected, id, dragging }: NodeProps<any>) => {
    ...
    const [connectedImage2, setConnectedImage2] = useState<string | undefined>(
      nodeData.connectedImage2
    );
    const [connectedImage3, setConnectedImage3] = useState<string | undefined>(
      nodeData.connectedImage3
    );
    const [connectedImage4, setConnectedImage4] = useState<string | undefined>(
      nodeData.connectedImage4
    );
    const [isBrandActive, setIsBrandActive] = useState<boolean>(
      nodeData.isBrandActive !== undefined
    ...
    const { debouncedUpdate: debouncedUpdateData } = useNodeDataUpdater<PromptNodeData>(
      nodeData.onUpdateData,
      id
    );
    ...
    const legacyImages = [connectedImage1, connectedImage2, connectedImage3, connectedImage4]
      .filter((img) => img && !brandCoreImages.includes(img))
      .slice(0, maxHandles);
    ...
    const handleGenerate = async () => {
      ...
      if (nodeData.onUpdateData) {
        ...
        if (hasOutputConfig && aspectRatio !== nodeData.aspectRatio)
      ...
      const finalPrompt =
        isBrandActive && nodeData.connectedTextDirection
          ? prompt
            ? `${nodeData.connectedTextDirection}\n\n${prompt}`
            : nodeData.connectedTextDirection
          : prompt;

      await nodeData.onGenerate(
        id,
        finalPrompt.trim(),
        ...
        {hasBrandCoreConnection && (
          <div className="node-margin">
            <Tooltip
              content={
                isBrandActive
                  ? 'Brand assets will be used in generation'
                  : 'Brand assets connected but inactive'
              }
            >
              <div
                className={cn(
              ...
              >
                <div className="relative flex items-center justify-center shrink-0">
                  <Palette
                    size={10}
                    className={cn(
                      isBrandActive ? 'text-brand-cyan' : 'text-neutral-600',
                      'transition-colors'
                    )}
                  />
                  {isBrandActive && (
