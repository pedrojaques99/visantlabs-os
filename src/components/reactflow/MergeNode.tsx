...
export const MergeNode: React.FC<NodeProps<Node<MergeNodeData>>> = memo(
  ({ data, selected, id, dragging }) => {
    ...
    const { resolution: effectiveResolution } = resolveGenerationContext(model, {
    ...
    const { debouncedUpdate: debouncedUpdateData } = useNodeDataUpdater<MergeNodeData>(
      data.onUpdateData,
      id
    );
    ...
    return (
      ...
      >
        ...
        <Handle type="target" position={Position.Left} id="input-1" className="node-handle" />
        <Handle type="target" position={Position.Left} id="input-2" className="node-handle" />
