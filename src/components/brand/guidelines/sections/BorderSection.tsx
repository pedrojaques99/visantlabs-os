  ...
  return (
    ...
    >
      <div className="space-y-1.5 py-1">
        ...
        {items.map((b, bi) => (
          <div key={b.id} className="group/border border-b border-neutral-800 last:border-0 overflow-hidden">
            {/* Always visible */}
            <div className="flex items-center gap-2 p-2">
              ...
              <span className="text-[10px] font-mono text-neutral-700">{b.width}px {b.style}</span>
              <Button variant="ghost" size="icon" className="h-5 w-5 text-neutral-800 hover:text-destructive opacity-0 group-hover/border:opacity-100 transition-all shrink-0" onClick={() => removeBorder(bi)} aria-label="Remove"><Trash2 size={10} /></Button>
            </div>
            {/* Hover-reveal */}
              ...
              <div className="px-2 pb-2 space-y-2 border-t border-neutral-800 pt-2">
                <div className="flex gap-1">
                  {(['solid', 'dashed', 'dotted'] as BorderStyle[]).map(s => (
                    <button key={s} type="button" onClick={() => update(bi, { style: s })}
                      className={cn('flex-1 h-6 rounded border text-[10px] font-mono uppercase transition-all', b.style === s ? 'border-white/20 bg-white/5 text-neutral-200' : 'border-neutral-800 text-neutral-600 hover:border-white/10')}
                    >{s}</button>
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-0.5 w-16">
                    ...
                    <Input type="number" step="0.5" min="0.5" value={b.width} onChange={e => update(bi, { width: Number(e.target.value) })} className="h-6 border-neutral-800 text-[10px] font-mono text-center" />
                  </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {(['default', 'emphasis', 'scaffold', 'divider'] as BorderRole[]).map(r => (
                    <button key={r} type="button" onClick={() => update(bi, { role: r })}
                      className={cn('px-2 h-5 rounded border text-[10px] font-mono transition-all', b.role === r ? 'border-white/20 bg-white/5 text-neutral-200' : 'border-neutral-800 text-neutral-600 hover:border-white/10')}
                    >{r}</button>
                  ))}
                </div>
              </div>
