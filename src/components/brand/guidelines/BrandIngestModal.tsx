...
const SectionShell: React.FC<{
  ...
  >
    ...
    >
      ...
      >
        {allChecked ? (
          <Check size={10} className="text-neutral-300" />
        ) : someChecked ? (
...
const ColorsSection: React.FC<{ data: any[]; sel: Set<number>; toggle: (i: number) => void }> = ({
  data,
  sel,
  toggle,
}) => (
...
const TypographySection: React.FC<{
  ...
  <div className="space-y-0">
    {data.map((f, i) => (
      <ItemCheck key={i} checked={sel.has(i)} onToggle={() => toggle(i)} dim>
        <span
          className="text-xs text-neutral-200 flex-shrink-0"
          style={{ fontFamily: f.family, minWidth: '8rem' }}
        >
          {f.family}
        </span>
...
const GradientsSection: React.FC<{
  data: any[];
  sel: Set<number>;
  toggle: (i: number) => void;
}> = ({ data, sel, toggle }) => (
  <div className="space-y-0">
    {data.map((g, i) => (
      <ItemCheck key={i} checked={sel.has(i)} onToggle={() => toggle(i)} dim>
        <div
          className="w-12 h-4 rounded border border-neutral-800 flex-shrink-0"
...
const ShadowsSection: React.FC<{ data: any[]; sel: Set<number>; toggle: (i: number) => void }> = ({
  data,
  sel,
  toggle,
}) => (
  <div className="space-y-0">
    {data.map((s, i) => (
      <ItemCheck key={i} checked={sel.has(i)} onToggle={() => toggle(i)} dim>
        <div
          className="w-5 h-5 rounded bg-neutral-800 flex-shrink-0"
...
const BordersSection: React.FC<{ data: any[]; sel: Set<number>; toggle: (i: number) => void }> = ({
  data,
  sel,
  toggle,
}) => (
  <div className="space-y-0">
    {data.map((b, i) => (
      <ItemCheck key={i} checked={sel.has(i)} onToggle={() => toggle(i)} dim>
        <div
          className="w-10 h-4 rounded flex-shrink-0 bg-neutral-900"
...
const RadiiSection: React.FC<{ data: number[]; sel: Set<number>; toggle: (i: number) => void }> = ({
  data,
  sel,
  toggle,
}) => (
...
export const BrandIngestModal: React.FC<BrandIngestModalProps> = ({
  ...
  useEffect(() => {
    ...
    state.gradients?.length,
    state.shadows?.length,
    state.borders?.length,
    state.radii?.length,
  ...
  const apply = async () => {
    ...
    try {
      if (!guideline.id) {
        toast.error('Guideline ID missing');
        return;
      }
      ...
      const typography = typographySel.length
        ? typographySel
        : pick('typography', state.typography);
      ...
      if (selStrat.length) {
        ...
        if (description)
          update.identity = { ...(update.identity || ex.identity || {}), description };
        ...
        if (claims.length) {
          ...
          if (newDos.length)
            update.guidelines = { ...(ex.guidelines || {}), dos: [...existingDos, ...newDos] };
        }
      ...
      queryClient.setQueryData(['brand-guidelines'], (old: any) => {
        ...
        return old.map((g: any) =>
          g.id === latestGuideline.id ? { ...latestGuideline, _id: latestGuideline.id } : g
        );
      });
  ...
  const renderContent = (key: FigCategory) => {
    ...
    switch (key) {
      case 'colors':
        return <ColorsSection data={items} sel={sel} toggle={toggle} />;
      case 'typography':
        return <TypographySection data={items} sel={sel} toggle={toggle} />;
      case 'gradients':
        return <GradientsSection data={items} sel={sel} toggle={toggle} />;
      case 'shadows':
        return <ShadowsSection data={items} sel={sel} toggle={toggle} />;
      case 'borders':
        return <BordersSection data={items} sel={sel} toggle={toggle} />;
      case 'radii':
        return <RadiiSection data={items} sel={sel} toggle={toggle} />;
      case 'strategy':
        return <StrategySection items={items} sel={sel} toggle={toggle} />;
      case 'components':
        return <ComponentsSection data={items} sel={sel} toggle={toggle} />;
      case 'images':
        return <AssetsSection data={items} sel={sel} toggle={toggle} />;
    }
  };
