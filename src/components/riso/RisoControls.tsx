...
const INK_CATEGORIES: Record<string, string[]> = {
  'Reds': ['Bright Red', 'Red', 'Crimson', 'Scarlet', 'Tomato', 'Marine Red', 'Cranberry', 'Maroon', 'Raspberry Red', 'Brick', 'Coral', 'Paprika'],
  'Pinks': ['Fluorescent Pink', 'Bubble Gum', 'Light Mauve', 'Dark Mauve', 'Orchid'],
  'Blues': ['Blue', 'Medium Blue', 'Riso Federal Blue', 'Cornflower', 'Sky Blue', 'Sea Blue', 'Lake', 'Indigo', 'Midnight', 'Steel'],
  'Greens': ['Green', 'Hunter Green', 'Kelly Green', 'Grass', 'Forest', 'Spruce', 'Moss', 'Ivy', 'Pine', 'Bright Olive Green', 'Fluorescent Green', 'Emerald'],
              ...
              <ToolPanelChip key={name} onClick={() => {
                ...
                <div className="flex items-center gap-2">
                  <div className="flex gap-0.5 shrink-0">
                    {colors.map((c, ci) => <div key={ci} className="w-3 h-3 rounded-full" style={{ backgroundColor: c }} />)}
                  </div>
        ...
        <div className="relative w-full">
          ...
          {exportOpen && (
            <div className="absolute bottom-full left-0 right-0 mb-1 bg-neutral-900 border border-neutral-700 rounded-lg p-1 shadow-xl z-20 animate-fade-in">
              {onExportSvg && (
                <button onClick={() => { onExportSvg(); setExportOpen(false); }} className="flex items-center gap-2 w-full px-3 py-2 rounded-md text-[11px] text-neutral-300 hover:bg-neutral-800 transition-colors">
                  <FileType size={14} className="text-neutral-500" />
                  Export SVG (Vector)
                </button>
              )}
              {onExportHiRes && (
                <button onClick={() => { onExportHiRes(); setExportOpen(false); }} className="flex items-center gap-2 w-full px-3 py-2 rounded-md text-[11px] text-neutral-300 hover:bg-neutral-800 transition-colors">
