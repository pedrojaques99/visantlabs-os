...
export const PRESETS = [
  ...
  },
  {
    ...
    mainLightColor: '#52ddeb',
    mainLightIntensity: 36,
    spotLightColor: '#2299aa',
  },
  {
    name: 'violet',
    emissive: '#aa88ff',
    emissiveIntensity: 0.25,
    mainLightColor: '#9966ee',
    mainLightIntensity: 36,
    spotLightColor: '#6633cc',
  },
  {
    name: 'amber',
    emissive: '#ffcc66',
    emissiveIntensity: 0.25,
    mainLightColor: '#ddaa44',
    mainLightIntensity: 38,
    spotLightColor: '#bb8822',
  },
  {
    name: 'rose',
    emissive: '#ffaacc',
    emissiveIntensity: 0.22,
    mainLightColor: '#ee8899',
    mainLightIntensity: 36,
    spotLightColor: '#cc5566',
  },
  {
    name: 'green',
    emissive: '#88ffcc',
    emissiveIntensity: 0.22,
    mainLightColor: '#55ddaa',
    mainLightIntensity: 36,
    spotLightColor: '#33aa77',
  },
  {
    name: 'blue',
    emissive: '#88aaff',
    emissiveIntensity: 0.25,
    mainLightColor: '#6688ee',
    mainLightIntensity: 36,
    spotLightColor: '#4455cc',
  },
  {
    name: 'warm',
    emissive: '#ffddbb',
    emissiveIntensity: 0.2,
    mainLightColor: '#eeccaa',
    mainLightIntensity: 40,
    spotLightColor: '#ccaa88',
  },
] as const;
...
const MouseLight = ({ isMobile, preset }: { isMobile: boolean; preset: Preset }) => {
  ...
  useEffect(() => {
    if (mainRef.current) {
      ...
      mainRef.current.intensity = isMobile
        ? preset.mainLightIntensity / 2
        : preset.mainLightIntensity;
    }
  }, [preset, isMobile]);
  ...
  useFrame((state) => {
    ...
    if (secRef.current) {
      secRef.current.position.x = THREE.MathUtils.lerp(
        secRef.current.position.x,
        state.mouse.x * -4,
        0.03
      );
      secRef.current.position.y = THREE.MathUtils.lerp(
        secRef.current.position.y,
        state.mouse.y * -4,
        0.03
      );
  ...
  return (
    <>
      <pointLight
        ref={mainRef}
        intensity={isMobile ? preset.mainLightIntensity / 2 : preset.mainLightIntensity}
        color={preset.mainLightColor}
        distance={15}
        decay={2}
      />
      {!isMobile && (
        <pointLight ref={secRef} intensity={15} color="#ffffff" distance={10} decay={1} />
      )}
    </>
  );
};
...
const SpotLightReactive = ({ preset, isMobile }: { preset: Preset; isMobile: boolean }) => {
  ...
  return (
    <spotLight
      ...
      angle={0.2}
      penumbra={1}
      intensity={isMobile ? 30 : 60}
      color={preset.spotLightColor}
    />
  );
};
    ...
    <div ref={wrapperRef} className={fullScreen ? 'fixed inset-0 z-[1]' : 'w-full h-full'}>
      ...
      >
        ...
        >
          ...
          >
            <Float
              speed={isMobile ? 1 : GLASS_BASE.floatSpeed}
              rotationIntensity={GLASS_BASE.floatRotation}
              floatIntensity={GLASS_BASE.floatIntensity}
            >
              <LogoMesh
                isMobile={isMobile}
