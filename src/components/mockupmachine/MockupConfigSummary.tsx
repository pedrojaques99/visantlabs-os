...
export const MockupConfigSummary: React.FC = () => {
  ...
  if (!selectedModel) {
    return <span className="text-2xs font-mono text-neutral-700 tracking-wider">—</span>;
  }
    ...
    <div className="flex items-center gap-1 flex-wrap">
      {items.map((item, i) => (
        <React.Fragment key={i}>
          {i > 0 && <span className="text-2xs text-neutral-700">·</span>}
          <span className="text-2xs font-mono text-neutral-500 tracking-wider">{item}</span>
        </React.Fragment>
      ))}
    </div>
  );
