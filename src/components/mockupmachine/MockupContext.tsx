...
export const MockupProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [generationIds, setGenerationIds] = useState<(string | null)[]>(
    Array(initialMockupCount).fill(null)
  );
  ...
  const [surpriseMePool, setSurpriseMePool] = useState<SurpriseMeSelectedTags>(() =>
    getSurpriseMeSelectedTags()
  );
  ...
  const value = useMemo(
    () => ({
      ...
      selectedVibeSegment,
      setSelectedVibeSegment,
      selectedVibeStyle,
      setSelectedVibeStyle,
      mockups,
      setMockups,
      isLoading,
      setIsLoading,
