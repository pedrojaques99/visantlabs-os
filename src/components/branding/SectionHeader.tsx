  ...
  return (
    <div className="flex-1">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className={`font-semibold font-manrope text-lg ${theme === 'dark' ? 'text-zinc-200' : 'text-zinc-800'
          }`}>{stepTitle}</h3>
      </div>
      {!isCollapsed && !hasData && (
        <div className="flex flex-wrap items-center gap-3 text-xs font-mono">
          <div className={`flex items-center gap-1.5 ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-600'
            }`}>
          ...
          {dependencies.length > 0 && (
            <div className="flex items-center gap-1.5">
              <Link2 className={`h-3.5 w-3.5 ${theme === 'dark' ? 'text-zinc-500' : 'text-zinc-500'
                }`} />
