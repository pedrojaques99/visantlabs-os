...
const SectionCard: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  return (
    <div
      ...
      } border rounded-xl p-5 ${className}`}
    >
      {children}
    </div>
  );
...
export const CentralMessageSection: React.FC<{ data: CentralMessage; pillars: BrandPillar[] }> = ({
  data,
  pillars,
}) => {
  const { theme } = useTheme();
...
export const MarketResearchV2Section: React.FC<{ data: MarketResearchV2 }> = ({ data }) => {
  ...
  const layers = [
    {
      label: t('branding.visant.whatCompetitorsDoWell'),
      items: data.whatCompetitorsDoWell,
      color: 'green',
    },
...
export const ArchetypesV2Section: React.FC<{
  archetypes: ArchetypesV2;
  toneOfVoice: ToneOfVoicePillar[];
}> = ({ archetypes, toneOfVoice }) => {
  const { theme } = useTheme();
  const { t } = useTranslation();
