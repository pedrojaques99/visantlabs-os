...
export const BrandingExpertChat: React.FC<BrandingExpertChatProps> = ({
  ...
  const [messages, setMessages] = useState<ExpertChatMessage[]>([
    {
      ...
      parts: [
        {
          text: 'Olá! Sou seu Especialista em Branding da Visant Labs. Como posso ajudar com sua marca hoje? Você pode subir manuais, referências ou me perguntar sobre sua estratégia.',
        },
      ],
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (currentFiles.length > 0) {
        ...
        try {
          for (const file of currentFiles) {
            ...
            await chatApi.uploadToSession(sessionId, parts, {
              fileName: file.name,
              fileType: file.type,
            });
          }
      ...
      try {
        setToolsBeingUsed([]);
        const { reply, toolsUsed, generationId } = await chatApi.sendMessage(
          sessionId,
          currentInput
        );
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className={cn(
              mode === 'modal'
                ? 'w-full h-screen md:h-[85vh] md:max-w-4xl flex shadow-2xl'
                : 'w-full h-full flex'
            )}
          >
            {/* Sidebar */}
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-1">
                {loadingSessions ? (
                  ...
                  <div className="text-xs text-neutral-500 p-2 text-center">Nenhuma sessão</div>
                ) : (
            ...
            <div className="flex flex-col flex-1 min-w-0">
              ...
              >
                ...
                {/* Messages */}
                <div
                  className="flex-1 overflow-y-auto p-8 md:px-12 md:py-10 space-y-8 scrollbar-thin overflow-x-hidden"
                  ref={scrollRef}
                >
                  {messages.map((msg, i) => (
                    <ChatMessage
