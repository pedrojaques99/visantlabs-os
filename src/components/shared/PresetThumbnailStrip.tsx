...
export const PresetThumbnailStrip: React.FC<PresetThumbnailStripProps> = React.memo(({
  ...
  return (
    <div className="shrink-0 border-b border-neutral-800/50">
      <div className="flex gap-1.5 px-3 py-2.5 overflow-x-auto scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-transparent">
        {presets.map((preset) => (
          <button
            key={preset.name}
            onClick={() => onSelect(preset.name)}
            className={cn(
              'shrink-0 flex flex-col items-center gap-1 group transition-all duration-150',
              activePreset === preset.name && 'ring-1 ring-white/20 rounded-md'
            )}
          >
