...
export const ModePreviewSwitcher: React.FC<ModePreviewSwitcherProps> = React.memo(({ mode, onChange }) => {
  const sourceUrl = useImageLabStore((s) => s.sourceUrl);
  const [previews, setPreviews] = useState<Record<string, string>>({});

  const generatePreviews = useCallback(async (url: string) => {
    if (!url) { setPreviews({}); return; }
        ...
        if (rRenderer.init()) {
          ...
          if (settings.layers.length === 0) {
            ...
            const layers = colors.map((c, i) => ({
              ...
              visible: true, alpha: 0.85, angle: i * 22.5,
              offsetX: [1, -1, 1][i], offsetY: [-1, 1, 1][i],
            }));
