...
export function generateHalftoneSvg(
  ...
  for (const ch of channels) {
    ...
    const gridCols = Math.ceil(diag / cellSize) + 2;
    const gridRows = Math.ceil(diag / cellSize) + 2;
    const offsetX = w / 2;
    const offsetY = h / 2;
    ...
    for (let row = -gridRows; row <= gridRows; row++) {
      for (let col = -gridCols; col <= gridCols; col++) {
        ...
        if (settings.randomness > 0) {
          const rx = hash(col, row) - 0.5;
          const ry = hash(col + 17, row + 31) - 0.5;
          gx += rx * settings.randomness * cellSize * 0.8;
          gy += ry * settings.randomness * cellSize * 0.8;
        }
  ...
  if (blendMode !== 'normal') {
    parts.push(`<g style="mix-blend-mode:${blendMode}">`);
  }
