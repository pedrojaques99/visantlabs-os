...
function renderFieldOverlay(
  ...
  if (overlay === 'velocity') {
    for (let j = 1; j <= N; j++)
      for (let i = 1; i <= N; i++) maxVal = Math.max(maxVal, solver.getSpeed(i, j));
    maxVal = Math.max(maxVal, 1);
  } else if (overlay === 'pressure') {
    for (let j = 1; j <= N; j++)
      for (let i = 1; i <= N; i++) maxVal = Math.max(maxVal, Math.abs(solver.getPressure(i, j)));
    maxVal = Math.max(maxVal, 0.001);
  } else if (overlay === 'vorticity') {
    for (let j = 2; j < N; j++)
      for (let i = 2; i < N; i++) maxVal = Math.max(maxVal, Math.abs(solver.getVorticity(i, j)));
    maxVal = Math.max(maxVal, 0.001);
  }
...
function renderVelocityArrows(
  ...
  for (let j = 1; j <= N; j += step)
    for (let i = 1; i <= N; i += step) maxSpeed = Math.max(maxSpeed, solver.getSpeed(i, j));
...
function renderExportMetadata(
  ...
  const lines = [
    ...
    `Obstacle: ${cfg.obstacleType}${cfg.obstacleType === 'text' ? ` "${cfg.text}"` : ''}  Scale: ${
      cfg.obstacleScale
    }%  Color: ${cfg.colorMode}  Render: ${cfg.renderMode}`,
  ...
  if (cfg.obstacleType === 'image' && cfg.obstacleImage) {
    ...
    switch (cfg.obstacleType) {
      ...
      case 'square':
        ...
        break;
      case 'diamond':
        ctx.moveTo(cx, cy - ry * 1.2);
        ctx.lineTo(cx + rx * 1.2, cy);
...
export const WindTunnelCanvas = forwardRef<
  WindTunnelHandle,
  {
...
>(({ config, onConfigChange }, ref) => {
  ...
  const rebuildObstacles = useCallback((width: number, height: number) => {
    ...
    solver.clearObstacles();
    for (let j = 1; j <= N; j++)
      for (let i = 1; i <= N; i++) solver.setObstacle(i, j, obstacles[i + (N + 2) * j]);
  ...
  useEffect(() => {
    ...
    config.bold,
    config.obstacleType,
    config.obstacleImage,
    config.obstacleScale,
    config.obstacleOffsetX,
    config.obstacleOffsetY,
    rebuildObstacles,
  ]);
    ...
    (e: React.TouchEvent) => {
      ...
      if (e.touches.length === 1) {
        ...
        touchRef.current = {
          ...
          startDist: 0,
          startScale: configRef.current.obstacleScale,
        };
