...
export const CommunityPresetsPage: React.FC = () => {
  ...
  const fetchMy = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${COMMUNITY_API}/my`, {
  ...
  const sorted = useMemo(() => {
    ...
    switch (sortKey) {
      ...
      case 'likes':
        return arr.sort((a, b) => (b.likesCount ?? 0) - (a.likesCount ?? 0));
      case 'used':
  ...
  const handleSave = useCallback(
    async (data: PresetFormData) => {
      ...
      try {
        ...
        toast.success(
          isCreating
            ? t('communityPresets.messages.presetCreated')
            : t('communityPresets.messages.presetUpdated')
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const updatePreset = useCallback(
    (id: string, patch: Partial<CommunityPreset>) => {
      const mapper = (prev: CommunityPreset[]) =>
        prev.map((p) => (p.id === id ? { ...p, ...patch } : p));
      if (viewMode === 'my') {
        setMyPresets(mapper);
      } else {
        setAllPresets(mapper);
      }
    },
    [viewMode]
  );
  ...
  const handleToggleLike = useCallback(
    async (id: string) => {
      ...
      updatePreset(id, {
        ...
        likesCount: wasLiked ? Math.max(0, wasCount - 1) : wasCount + 1,
      });
      try {
        const res = await fetch(`${COMMUNITY_API}/${id}/like`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        updatePreset(id, { isLikedByUser: wasLiked, likesCount: wasCount });
      }
    },
                  ...
                  >
                    {rowItems.map((preset) => (
                      <PresetCard
                        ...
                        onToggleLike={
                          isAuthenticated ? () => handleToggleLike(preset.id) : undefined
                        }
