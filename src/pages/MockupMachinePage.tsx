...
const MockupMachinePageContent: React.FC = () => {
  ...
  const { hasEnoughCredits, validateCredits } = useCreditValidation(
    mockupCount,
    onCreditPackagesModalOpen
  );
  ...
  const [feedbackRatings, setFeedbackRatings] = useState<Map<number, FeedbackRating | null>>(
    new Map()
  );
  const handleFeedbackRatingChange = useCallback((index: number, rating: FeedbackRating | null) => {
    setFeedbackRatings((prev) => {
      const next = new Map(prev);
      if (rating === null) next.delete(index);
      else next.set(index, rating);
      return next;
    });
  }, []);
  ...
  const hasUnsavedImages = mockups.some(
    (mockup, index) => mockup !== null && !savedIndices.has(index)
  );
      ...
      if (model === GEMINI_MODELS.FLASH && previousModel === GEMINI_MODELS.PRO) {
        ...
        if (subscriptionStatus) {
          ...
          if (remaining < minCredits) {
            toast.warning(t('messages.modelRequiresCredits', { minCredits, remaining }), {
              duration: 4000,
            });
            onCreditPackagesModalOpen();
    ...
    (count: number = 1) => {
      ...
      setMockups((prev) => {
        ...
        const alreadyLoading =
          prev.length >= count &&
          prev.slice(0, count).every((m) => m === null) &&
          isLoading.slice(0, count).every((l) => l === true);
      ...
      setIsLoading((prev) => {
        const alreadyLoading =
          mockups.length >= count &&
          mockups.slice(0, count).every((m) => m === null) &&
          prev.slice(0, count).every((l) => l === true);
        if (alreadyLoading) return prev;
  ...
  const buildPrompt = useCallback(() => {
    ...
    switch (aspectRatio) {
      case '16:9':
        aspectInstruction = `widescreen cinematic shot`;
        break;
      case '4:3':
        aspectInstruction = `standard photo`;
        break;
      case '1:1':
        aspectInstruction = `square composition`;
        break;
      default:
        aspectInstruction = `image with an aspect ratio of ${aspectRatio}`;
        break;
    }
    let basePrompt = '';

    if (selectedAngleTags.length > 0)
      basePrompt += ` The camera angle should be: ${selectedAngleTags.join(', ')}.`;
    if (selectedLightingTags.length > 0)
      basePrompt += ` The lighting should be: ${selectedLightingTags.join(', ')}.`;
    if (selectedEffectTags.length > 0)
      basePrompt += ` Apply a visual effect of: ${selectedEffectTags.join(', ')}.`;
    if (selectedMaterialTags.length > 0)
      basePrompt += ` The materials and textures should feature: ${selectedMaterialTags.join(
        ', '
      )}.`;
    if (selectedColors.length > 0)
      basePrompt += ` The scene's color palette should be dominated by or feature accents of: ${selectedColors.join(
        ', '
      )}.`;

    // Add instruction about reference images if present
    if (referenceImages.length > 0) {
      basePrompt += ` IMPORTANT: The provided reference image${
        referenceImages.length > 1 ? 's' : ''
      } ${
        referenceImages.length > 1 ? 'are' : 'is'
      } included as style and composition guidance. Study ${
        referenceImages.length > 1 ? 'these images' : 'this image'
      } carefully and create a mockup that matches the aesthetic, mood, composition style, lighting approach, color palette, and overall visual feeling of ${
        referenceImages.length > 1 ? 'these reference mockups' : 'this reference mockup'
      }. Use ${
        referenceImages.length > 1 ? 'them' : 'it'
      } as inspiration for creating a similar visual quality and atmosphere.`;
    }

    if (designType === 'logo') {
      if (generateText)
        basePrompt +=
          ' If appropriate for the mockup type, generate plausible placeholder text to make the scene more realistic.';
      else
        basePrompt +=
          ' No additional text or letters should be generated. The design is the sole graphic element.';
    }
  ...
  const executeImageEditOperation = useCallback(
    async (params: {
      base64Image: string;
      prompt: string;
      ...
      try {
        ...
        if (result.isAdmin) {
          ...
          toast.success(
            `${t('credits.notificationUsed', { count: result.creditsDeducted, plural })}. ${t(
              'credits.notificationRemaining',
              { remaining: result.creditsRemaining, plural: remainingPlural }
            )}`
          );
        }
  ...
  const runGeneration = useCallback(
    async (indexToUpdate?: number, promptOverride?: string, appendMode: boolean = false) => {
      ...
      if (isLoading.some(Boolean) && !appendMode && indexToUpdate === undefined) {
        if (isLocalDevelopment()) {
          console.warn(
            '[runGeneration] Generation already in progress and not in append mode, ignoring duplicate call'
          );
        }
        return;
      }
      ...
      const hasValidSetup =
        hasReferenceImages || (designType && uploadedImage && designType !== 'blank');

      if (!hasValidSetup || !promptToUse.trim()) {
        if (isLocalDevelopment()) {
          console.warn('[runGeneration] Validation failed:', {
            hasValidSetup,
            promptLength: promptToUse.trim().length,
            designType,
            hasUploadedImage: !!uploadedImage,
            hasReferenceImages,
          });
      ...
      if (indexToUpdate !== undefined) {
        ...
        if (appendMode) {
          ...
          setMockups((prev) => {
            ...
            const alreadyHasLoadingSlots =
              prev.length >= mockupCount &&
          ...
          const promises = Array.from({ length: mockupCount }, (_, i) => generateAndSet(i));
          ...
          if (!isLocal) {
            try {
              ...
              if (isLocalDevelopment()) {
                console.error(
                  'Failed to refresh subscription status after append generation:',
                  statusError
                );
              }
            }
          ...
          if (successfulCount > 0 && !isLocal) {
            try {
              ...
              if (isLocalDevelopment()) {
                console.error(
                  'Failed to refresh subscription status after batch generation:',
                  statusError
                );
              }
            }
  ...
  const handleGenerateSmartPrompt = useCallback(
    async (shouldAutoGenerate: boolean = false) => {
      ...
      try {
        ...
        const smartPrompt =
          typeof smartPromptResult === 'string' ? smartPromptResult : smartPromptResult.prompt;
        ...
        if (referenceImages.length > 0) {
          finalPrompt += ` IMPORTANT: The provided reference image${
            referenceImages.length > 1 ? 's' : ''
          } ${
            referenceImages.length > 1 ? 'are' : 'is'
          } included as style and composition guidance. Study ${
        ...
        const genId =
          typeof smartPromptResult === 'object' && smartPromptResult.generationId
        ...
        setTimeout(() => {
          ...
          if (generateOutputsButtonRef.current && sidebarRef.current) {
            ...
            generateOutputsButtonRef.current?.scrollIntoView({
              behavior: 'smooth',
              block: 'center',
            });
          }
  ...
  const handleAnalyze = useCallback(
    async (imageOverride?: UploadedImage, silent?: boolean) => {
      const imageToUse = imageOverride ?? uploadedImage;
      const t0 = Date.now();
      if (import.meta.env.DEV)
        console.log('[dev] analyze: handleAnalyze start', silent ? '(silent)' : '');
      if (!imageToUse) {
      ...
      if (!silent) {
        ...
        nextStepTimeoutId = window.setTimeout(() => {
          if (import.meta.env.DEV)
            console.log(
              '[dev] analyze: showing next step after 5s (analysis may still be running)'
            );
      ...
      try {
        ...
        if (imageToUse.base64) {
          try {
            const dataUrl = await compressImage(
              imageToUse.base64.includes(',')
                ? imageToUse.base64
                : `data:${imageToUse.mimeType};base64,${imageToUse.base64}`,
              {
                maxWidth: 1024,
                maxHeight: 1024,
        ...
        if (import.meta.env.DEV)
          console.log(
            '[dev] analyze: aiApi.analyzeSetup start',
        ...
        if (import.meta.env.DEV)
          console.log(
            '[dev] analyze: aiApi.analyzeSetup done',
            ((Date.now() - t0) / 1000).toFixed(2) + 's'
          );
        ...
        if (imageToUse.base64) {
          try {
            if (import.meta.env.DEV)
              console.log(
                '[dev] analyze: extractColors start',
                ((Date.now() - t0) / 1000).toFixed(2) + 's'
              );
            ...
            if (import.meta.env.DEV)
              console.log(
                '[dev] analyze: extractColors done',
                ((Date.now() - t0) / 1000).toFixed(2) + 's'
              );
          } catch (colorErr) {
        ...
        if (import.meta.env.DEV)
          console.log(
            '[dev] analyze: handleAnalyze success',
            ((Date.now() - t0) / 1000).toFixed(2) + 's'
          );
      } catch (err) {
        const errorInfo = formatMockupError(err, t);
        ...
        if (import.meta.env.DEV)
          console.log(
            '[dev] analyze: handleAnalyze finally',
            ((Date.now() - t0) / 1000).toFixed(2) + 's'
          );
        if (!silent) {
  ...
  const handleSurpriseMe = useCallback(
    async (autoGenerate: boolean = false) => {
      ...
      if (hasVibeDirection) {
        ...
        console.log(
          '[SurpriseMe] Using vibe direction:',
          selectedVibeSegment,
          '×',
          selectedVibeStyle
        );
        ...
        setSelectedEffectTags(
          Math.random() < 0.6 ? pickFromVibe(vibeConfig.effectTags, AVAILABLE_EFFECT_TAGS) : []
        );
        setSelectedMaterialTags(
          designType === 'logo'
            ? pickFromVibe(vibeConfig.materialTags, AVAILABLE_MATERIAL_TAGS)
            : []
        );
      } else {
        // ─── LEGACY ARCHETYPE PATH (no vibe selected) ───
        // Use pool from Context when Pool Mode is active
        const selectedTagsSettings: SurpriseMeSelectedTags = isSurpriseMeMode
          ? surpriseMePool
          : {
        ...
        const filteredAiCategories = aiSuggestedCategories.filter(
          (tag) => userAllowedCategories.length === 0 || userAllowedCategories.includes(tag)
        );
        ...
        const filteredAiLocations = aiSuggestedLocations.filter(
          (tag) => userAllowedLocations.length === 0 || userAllowedLocations.includes(tag)
        );
        ...
        if (filteredAiLocations.length > 0) {
          ...
          const filteredBackgrounds = suitableBackgrounds.filter(
            (bg) =>
              bg !== 'Nature landscape' &&
              (userAllowedLocations.length === 0 || userAllowedLocations.includes(bg))
          );
          const preferredOptions = ['Light Box', 'Minimalist Studio'];
          const filteredPreferred =
        ...
        try {
          ...
          const pickTagWithVariety = (
            ...
            let pool =
              userAllowed.length > 0
                ? availableTags.filter((t) => userAllowed.includes(t))
                : availableTags;
            if (Math.random() < 0.8 && archetypeTags.length > 0) {
              const archetypePool = archetypeTags.filter(
                (t) => userAllowed.length === 0 || userAllowed.includes(t)
              );
          ...
          if (currentArchetype.visuals.locations) {
            selectedBackground = pickTagWithVariety(
              currentArchetype.visuals.locations,
              AVAILABLE_LOCATION_TAGS,
              userAllowedLocations
            );
          }
          setSelectedLocationTags([
            selectedBackground || selectRandomBackground(brandingTagsToUse),
          ]);

          const userAllowedLighting = selectedTagsSettings.selectedLightingTags;
          let lightingTag = currentArchetype.visuals.lighting
            ? pickTagWithVariety(
                currentArchetype.visuals.lighting,
                AVAILABLE_LIGHTING_TAGS,
                userAllowedLighting
              )
            : '';
          if (!lightingTag) {
            const pool =
              userAllowedLighting.length > 0 ? userAllowedLighting : AVAILABLE_LIGHTING_TAGS;
          ...
          if (Math.random() < 0.5) {
            let effectTag = currentArchetype.visuals.effects
              ? pickTagWithVariety(
                  currentArchetype.visuals.effects,
                  AVAILABLE_EFFECT_TAGS,
                  userAllowedEffects
                )
              : '';
            if (!effectTag) {
              const pool =
                userAllowedEffects.length > 0 ? userAllowedEffects : AVAILABLE_EFFECT_TAGS;
              effectTag = rand(pool);
            }
          ...
          if (designType === 'logo') {
            ...
            let materialTag = currentArchetype.visuals.materials
              ? pickTagWithVariety(
                  currentArchetype.visuals.materials,
                  AVAILABLE_MATERIAL_TAGS,
                  userAllowedMaterials
                )
              : '';
            if (!materialTag) {
              const pool =
                userAllowedMaterials.length > 0 ? userAllowedMaterials : AVAILABLE_MATERIAL_TAGS;
          ...
          setSelectedMaterialTags(
            designType === 'logo' ? [rand([...AVAILABLE_MATERIAL_TAGS])] : []
          );
        }
      }
  ...
  const handleSurpriseMeWithDice = useCallback(
    (autoGen: boolean) => {
  ...
  const handleImageUpload = useCallback(
    async (image: UploadedImage) => {
      ...
      if (image.base64 && !image.url) {
        mockupApi
          .uploadTempImage(image.base64, image.mimeType)
          .then((url) =>
            loadImage(url, null)
              .then(() =>
  ...
  const handleReplaceImage = useCallback(
    async (image: UploadedImage) => {
      ...
      const validImage: UploadedImage = base64String
        ? { base64: base64String, mimeType }
        : { url: image.url!, mimeType };
      ...
      if (base64String && !image.url) {
        mockupApi
          .uploadTempImage(base64String, mimeType)
          .then((url) =>
            loadImage(url, null)
              .then(() =>
                setUploadedImage((prev) => (prev ? { ...prev, url, base64: undefined } : null))
    ...
    try {
      ...
      const savePromises = unsavedIndices.map(async (index) => {
        ...
        if (imageBase64) {
          ...
          if (savedMockup._id) {
            setMockupLikedStatus((prev) =>
              new Map(prev).set(index, savedMockup.isLiked ?? isLiked)
            );
          }
      ...
      toast.success(
        t('messages.savedOutputs', {
          count: unsavedIndices.length,
          plural: unsavedIndices.length > 1 ? 's' : '',
        }),
        { duration: 3000 }
      );
    } catch (error: any) {
  ...
  const handleGenerateClick = useCallback(async () => {
    ...
    if (!isPromptReady && !hasPrompt) {
      ...
      if (autoGenerate) {
        ...
        await runGeneration(
          undefined,
          generatedSmartPromptRef.current || undefined,
          hasExistingOutputs
        );
  ...
  const handleSaveMockup = useCallback(
    async (index: number, imageBase64: string) => {
      if (!isAuthenticated) {
  ...
  const handleLikeStateChange = useCallback(
    (index: number) => (newIsLiked: boolean) => {
      ...
      if (!mockupId || !isValidObjectId) {
        ...
        try {
          ...
          if (savedMockup._id) {
            ...
            // Update liked status from saved mockup
            setMockupLikedStatus((prev) =>
              new Map(prev).set(index, savedMockup.isLiked ?? newLikedState)
            );
          }

          toast.success(
            newLikedState ? t('canvas.addedToFavorites') : t('canvas.removedFromFavorites'),
            { duration: 2000 }
          );
        } catch (error: any) {
          if (isLocalDevelopment()) {
  ...
  const handleRemoveMockup = useCallback(
    (index: number) => {
  ...
  const handleRedrawFromModal = (index: number) => {
    handleRedrawClick(index);
    handleCloseFullScreen();
  };
  const handleZoomInFromModal = (index: number) => {
    handleCloseFullScreen();
    handleZoomInFromOutput(index);
  };
  const handleZoomOutFromModal = (index: number) => {
    handleCloseFullScreen();
    handleZoomOutFromOutput(index);
  };
  const handleNewAngleFromModal = (index: number, angle: string) => {
    handleCloseFullScreen();
    handleNewAngleFromOutput(index, angle);
  };
  const handleNewBackgroundFromModal = (index: number) => {
    handleCloseFullScreen();
    handleNewBackgroundFromOutput(index);
  };
  ...
  const handleNewAngleFromOutput = useCallback(
    async (index: number, angle: string) => {
    ...
    [
      ...
      isGeneratingPrompt,
      buildPrompt,
      executeImageEditOperation,
      prepareForNewMockupSlot,
    ]
  );

  const handleNewBackgroundFromOutput = useCallback(
    async (index: number) => {
      const outputImage = mockups[index];
      if (!outputImage || isLoading[index] || isGeneratingPrompt) return;
  ...
  const handleNewBackgroundFromOutputWithPreset = useCallback(
    async (index: number, background: string) => {
      const outputImage = mockups[index];
      if (!outputImage || isLoading[index] || isGeneratingPrompt) return;
  ...
  const handleNewLightingFromOutputWithPreset = useCallback(
    async (index: number, lighting: string) => {
      const outputImage = mockups[index];
      if (!outputImage || isLoading[index] || isGeneratingPrompt) return;
  ...
  const handleZoomInFromOutput = useCallback(
    async (index: number) => {
      const outputImage = mockups[index];
      if (!outputImage || isLoading[index] || isGeneratingPrompt) return;
  ...
  const handleZoomOutFromOutput = useCallback(
    async (index: number) => {
      const outputImage = mockups[index];
      if (!outputImage || isLoading[index] || isGeneratingPrompt) return;
  ...
  const handleReImagineFromOutput = useCallback(
    async (index: number, reimaginePrompt: string) => {
      const outputImage = mockups[index];
      if (!outputImage || isLoading[index] || isGeneratingPrompt) return;
      ...
      {fullScreenImageIndex !== null && (
        <FullScreenViewer
          ...
          onNewBackground={(background) =>
            handleNewBackgroundFromModalWithPreset(fullScreenImageIndex, background)
          }
          onNewLighting={(lighting) =>
            handleNewLightingFromModalWithPreset(fullScreenImageIndex, lighting)
          }
          onReImagine={(reimaginePrompt) =>
            handleReImagineFromOutput(fullScreenImageIndex, reimaginePrompt)
          }
