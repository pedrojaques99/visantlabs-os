...
async function callVideoApi(body: object): Promise<string> {
  ...
  if (!res.ok) {
    ...
    throw new Error(b.error || 'Video generation failed');
  }
...
const generateVideoFromMoodboard = (
...
function loadSession(): { sourceImage: string | null; croppedImages: CroppedImage[] } {
  try {
    ...
    return { sourceImage: null, croppedImages: [] };
  }
}
...
function MoodboardStudio() {
  ...
  useEffect(() => {
    saveSession(sourceImage, croppedImages);
  ...
  const handleAISuggest = useCallback(
    async (itemsToSuggest?: CroppedImage[]) => {
      const targets =
        itemsToSuggest ||
      ...
      try {
        const { suggestions } = await moodboardApi.suggest(
          targets.map((t) => ({ id: t.id, base64: t.url }))
        );
        setCroppedImages((prev) =>
          prev.map((crop) => {
            const s = suggestions.find((s) => s.id === crop.id);
            return s
              ? { ...crop, animationPrompt: s.prompt, suggestedPreset: s.preset as AnimationPreset }
              : crop;
          })
        );
      } catch (err: any) {
  ...
  const processFiles = useCallback(
    async (files: File[]) => {
      ...
      const newItems: CroppedImage[] = await Promise.all(
        valid.map(async (file) => {
          ...
          try {
            thumbnailUrl = await generateThumbnail(dataUrl, id);
          } catch {}
  ...
  const splitImage = async () => {
    ...
    try {
      ...
      if (boxes.length === 0) {
        toast.error(t('moodboard.studio.no_grid_detected_try_a_moodboard_'));
        return;
      }
      ...
      for (let i = 0; i < boxes.length; i++) {
        ...
        const x = (box.x / 100) * img.width,
          y = (box.y / 100) * img.height;
        const w = (box.width / 100) * img.width,
          h = (box.height / 100) * img.height;
        canvas.width = w;
        canvas.height = h;
        ...
        let thumbnailUrl: string | undefined;
        try {
          thumbnailUrl = await generateThumbnail(canvas.toDataURL('image/jpeg', 0.9), id);
        } catch {}
        newCrops.push({
          id,
  ...
  const handleUpscale = async (id: string) => {
    ...
    setCroppedImages((prev) =>
      prev.map((c) => (c.id === id ? { ...c, isUpscaling: true, upscaleStartTime: Date.now() } : c))
    );
    try {
      ...
      setCroppedImages((prev) =>
        prev.map((c) =>
          c.id === id ? { ...c, upscaledUrl: upscaledBase64, isUpscaling: false } : c
        )
      );
    } catch (err: any) {
  ...
  const handleAnimate = async (id: string, prompt: string) => {
    ...
    setCroppedImages((prev) =>
      prev.map((c) =>
        c.id === id ? { ...c, isAnimating: true, animationStartTime: Date.now() } : c
      )
    );
    try {
  ...
  const runRegenJob = useCallback(async ({ id, model, provider }: RegenJob) => {
    ...
    try {
      ...
      if (!res.ok) {
        const b = await res.json().catch(() => ({}));
        throw new Error(b.error || 'Regeneration failed');
      }
      ...
      toast.success(t('moodboard.studio.ai_regeneration_ready'), {
        ...
        action: {
          label: 'Download',
          onClick: () => downloadImage(regeneratedUrl, `regen-${id}-${Date.now()}.png`),
        },
        duration: 10000,
      });
    } catch (err: any) {
      toast.error(
        t('moodboard.studio.regen_failed', {
          id: id.slice(-4),
          error: err.message || t('moodboard.studio.regeneration_failed'),
        })
      );
    } finally {
  ...
  const handleRegenerate = useCallback(
    (id: string, model: string, provider: ImageProvider) => {
  ...
  const handleCreateFullVideo = async () => {
    ...
    try {
      ...
      const prompt =
        'A professional cinematic video showcasing these images with smooth transitions, professional lighting, and high aesthetic quality.';
  ...
  const handleBatchRemotion = async (preset: AnimationPreset) => {
    ...
    if (targets.length === 1) {
      ...
      setRemotionData({
        ...
        thumbnailUrl: crop.thumbnailUrl || url,
  ...
  const handleRemotionAnimate = async (
    url: string,
    preset: AnimationPreset,
    crop: CroppedImage,
    index: number
  ...
  const handleUpdateCardImage = async (id: string, file: File) => {
    ...
    reader.onload = async (e) => {
      ...
      let thumbnailUrl: string | undefined;
      try {
        thumbnailUrl = await generateThumbnail(url, id);
      } catch {}
  ...
  const downloadAll = () => {
    const targets =
      selectedIds.size > 0 ? croppedImages.filter((c) => selectedIds.has(c.id)) : croppedImages;
    targets.forEach((crop, i) =>
      setTimeout(
        () =>
          downloadImage(
            crop.upscaledUrl || crop.url,
            `item-${i + 1}${crop.upscaledUrl ? '-4k' : ''}.jpg`
          ),
        i * 300
      )
    );
  ...
  const handleRegenAll = () => {
    const targets =
      selectedIds.size > 0
        ? croppedImages.filter((c) => selectedIds.has(c.id) && c.url)
        : croppedImages.filter((c) => c.url);
    enqueueRegenAll(
      targets.map((c) => ({ id: c.id, model: batchRegenModel, provider: batchRegenProvider }))
    );
  };
          ...
          >
            ...
            <div className="flex gap-3">
              <Button variant="default" onClick={splitImage} disabled={isAnalyzing}>
                {isAnalyzing ? (
                  <GlitchLoader size={15} className="mr-2" />
                ) : (
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                {croppedImages.length > 0 && (
                  ...
                  >
                    {isCreatingFullVideo ? (
                      <GlitchLoader size={13} className="mr-1.5" />
                    ) : (
              ...
              <div className="flex items-center gap-2 flex-wrap justify-end">
                <ModelSelector
                  ...
                  onModelChange={(m, p) => {
                    setBatchRegenModel(m);
                    if (p) setBatchRegenProvider(p);
                  }}
                  className="w-[160px]"
                />
                <Button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {croppedImages.map((crop, idx) => (
                <BentoItem
                  ...
                  onAcceptRegenerated={(id) =>
                    setCroppedImages((prev) =>
                      prev.map((c) =>
                        c.id === id && c.regeneratedUrl
                          ? {
                              ...c,
                              url: c.regeneratedUrl,
                              thumbnailUrl: undefined,
                              upscaledUrl: undefined,
                              regeneratedUrl: undefined,
                            }
                          : c
                      )
                    )
                  }
                  onDiscardRegenerated={(id) =>
                    setCroppedImages((prev) =>
                      prev.map((c) => (c.id === id ? { ...c, regeneratedUrl: undefined } : c))
                    )
                  }
                />
              ))}
