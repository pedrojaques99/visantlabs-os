...
export const CompressPage: React.FC = () => {
          ...
          <div className="flex gap-1">
            {DIMENSION_OPTIONS.map((d) => (
              <button
                ...
                disabled={isProcessing}
                className={cn(
                  'flex-1 px-2 py-1 rounded text-xs font-mono transition-all duration-200 tabular-nums',
                  maxDimension === d
          ...
          <div className="flex gap-1">
            {FORMAT_OPTIONS.map((f) => (
              <button
                ...
                disabled={isProcessing}
                className={cn(
                  'flex-1 px-2 py-1 rounded text-xs font-mono uppercase transition-all duration-200',
                  outputFormat === f
                    ? 'bg-brand-cyan/20 text-brand-cyan border border-brand-cyan/40'
                    : 'bg-neutral-900 text-neutral-500 border border-neutral-800 hover:border-neutral-600'
                )}
  ...
  return (
    <MiniAppShell
      ...
      documentTitle="Image Compressor"
