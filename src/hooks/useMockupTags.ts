...
export const useMockupTags = () => {
  ...
  // Dynamic Tag State
  const [availableBrandingTags, setAvailableBrandingTags] =
    useState<string[]>(AVAILABLE_BRANDING_TAGS);
  const [availableMockupTags, setAvailableMockupTags] = useState<string[]>(AVAILABLE_TAGS);
  const [availableLocationTags, setAvailableLocationTags] =
    useState<string[]>(AVAILABLE_LOCATION_TAGS);
  const [availableAngleTags, setAvailableAngleTags] = useState<string[]>(AVAILABLE_ANGLE_TAGS);
  const [availableLightingTags, setAvailableLightingTags] =
    useState<string[]>(AVAILABLE_LIGHTING_TAGS);
  const [availableEffectTags, setAvailableEffectTags] = useState<string[]>(AVAILABLE_EFFECT_TAGS);
  const [availableMaterialTags, setAvailableMaterialTags] =
    useState<string[]>(AVAILABLE_MATERIAL_TAGS);
  ...
  const handleBrandingTagToggle = useCallback(
    (tag: string) => {
  ...
  const handleLocationTagToggle = useCallback(
    (tag: string) => setSelectedLocationTags(selectedLocationTags.includes(tag) ? [] : [tag]),
    [selectedLocationTags, setSelectedLocationTags]
  );

  const handleAngleTagToggle = useCallback(
    (tag: string) => setSelectedAngleTags(selectedAngleTags.includes(tag) ? [] : [tag]),
    [selectedAngleTags, setSelectedAngleTags]
  );

  const handleLightingTagToggle = useCallback(
    (tag: string) => setSelectedLightingTags(selectedLightingTags.includes(tag) ? [] : [tag]),
    [selectedLightingTags, setSelectedLightingTags]
  );

  const handleEffectTagToggle = useCallback(
    (tag: string) => setSelectedEffectTags(selectedEffectTags.includes(tag) ? [] : [tag]),
    [selectedEffectTags, setSelectedEffectTags]
  );

  const handleMaterialTagToggle = useCallback(
    (tag: string) => setSelectedMaterialTags(selectedMaterialTags.includes(tag) ? [] : [tag]),
  ...
  const randomizeSelection = useCallback(
    (available: string[], setter: (tags: string[]) => void, count: number = 1) => {
  ...
  const randomizeBranding = useCallback(
    () => randomizeSelection(availableBrandingTags, setSelectedBrandingTags, 1),
    [availableBrandingTags, setSelectedBrandingTags, randomizeSelection]
  );
  const randomizeCategory = useCallback(
    () => randomizeSelection(availableMockupTags, setSelectedTags, 1),
    [availableMockupTags, setSelectedTags, randomizeSelection]
  );
  const randomizeLocation = useCallback(
    () => randomizeSelection(availableLocationTags, setSelectedLocationTags, 1),
    [availableLocationTags, setSelectedLocationTags, randomizeSelection]
  );
  const randomizeAngle = useCallback(
    () => randomizeSelection(availableAngleTags, setSelectedAngleTags, 1),
    [availableAngleTags, setSelectedAngleTags, randomizeSelection]
  );
  const randomizeLighting = useCallback(
    () => randomizeSelection(availableLightingTags, setSelectedLightingTags, 1),
    [availableLightingTags, setSelectedLightingTags, randomizeSelection]
  );
  const randomizeEffect = useCallback(
    () => randomizeSelection(availableEffectTags, setSelectedEffectTags, 1),
    [availableEffectTags, setSelectedEffectTags, randomizeSelection]
  );
  const randomizeMaterial = useCallback(
    () => randomizeSelection(availableMaterialTags, setSelectedMaterialTags, 1),
    [availableMaterialTags, setSelectedMaterialTags, randomizeSelection]
  );
  ...
  const togglePoolTag = useCallback(
    (
      ...
      const newPool = {
        ...pool,
        [category]: isInPool ? currentTags.filter((t: string) => t !== tag) : [...currentTags, tag],
      };
      setPool(newPool);
