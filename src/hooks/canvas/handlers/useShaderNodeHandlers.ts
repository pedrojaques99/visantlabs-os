...
import {
  processImageOrVideoWithShader,
  isVideoInput,
} from '@/hooks/canvas/utils/shaderProcessingUtils';
...
export const useShaderNodeHandlers = ({
  ...
  const handleShaderApply = useCallback(
    async (nodeId: string, imageInput: string) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      if (!node || node.type !== 'shader') {
        console.warn('handleShaderApply: Node not found or wrong type', {
          nodeId,
          foundNode: !!node,
        });
        return;
      ...
      const settings = {
        ...
        duotoneShadowColor:
          shaderData.duotoneShadowColor ?? ([0.1, 0.0, 0.2] as [number, number, number]),
        duotoneHighlightColor:
          shaderData.duotoneHighlightColor ?? ([0.3, 0.9, 0.9] as [number, number, number]),
  ...
  const handleShaderNodeDataUpdate = useNodeDataUpdateHandler<ShaderNodeData>(
    updateNodeData,
    'shader'
  );
