...
export const useVideoNodeHandlers = ({
  ...
  const handleVideoNodeDataUpdate = useCallback(
    (nodeId: string, newData: Partial<VideoNodeData>) => {
      updateNodeData<VideoNodeData>(nodeId, newData, 'video');
    },
    [updateNodeData]
  );

  const handleVideoInputNodeDataUpdate = useCallback(
    (nodeId: string, newData: Partial<VideoInputNodeData>) => {
      updateNodeData<VideoInputNodeData>(nodeId, newData, 'videoInput');
    },
    [updateNodeData]
  );

  const handleVideoInputNodeUpload = useCallback(
    async (nodeId: string, videoData: string | File) => {
      ...
      try {
        const processMediaInput = async (
          ...
          if (input && typeof input === 'object') {
            if (input.file) {
              const r = await videoToBase64(input.file);
              return r.base64 ? `data:${r.mimeType};base64,${r.base64}` : undefined;
            }
        ...
        if (params.mode === 'frames_to_video') {
          ...
          if (refs.length === 0) {
            [
              videoData.connectedImage1,
              videoData.connectedImage2,
              videoData.connectedImage3,
              videoData.connectedImage4,
            ]
              .filter(Boolean)
              .forEach((img) => refs.push(img!));
          }
