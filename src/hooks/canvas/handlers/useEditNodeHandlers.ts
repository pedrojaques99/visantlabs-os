...
export const useEditNodeHandlers = ({
  ...
  const handleEditNodeDataUpdate = useCallback(
    (nodeId: string, newData: Partial<EditNodeData>) => {
      updateNodeData<EditNodeData>(nodeId, newData, 'edit');
    },
    [updateNodeData]
  );

  const handleUpscaleNodeDataUpdate = useCallback(
    (nodeId: string, newData: Partial<UpscaleNodeData>) => {
  ...
  const handleEditApply = useCallback(
    async (nodeId: string, imageBase64: string, config: EditNodeData) => {
      console.log('handleEditApply called', {
        nodeId,
        imageBase64: imageBase64 ? 'provided' : 'empty',
        hasConfig: !!config,
      });

      const node = nodesRef.current.find((n) => n.id === nodeId);
      if (!node || node.type !== 'edit') {
        console.warn('handleEditApply: Node not found or wrong type', {
          nodeId,
          foundNode: !!node,
        });
        return;
      ...
      if (!connectedEdge) {
        console.warn('handleEditApply: No connected edge found', {
          nodeId,
      ...
      console.log('handleEditApply: Proceeding with edit', {
        nodeId,
      ...
      const rawPrompt =
        ...
        'Apply the requested changes to this image while maintaining its overall composition and quality.';
      ...
      try {
        ...
        if (newOutputNodeId) {
          updateOutputNodeWithResult(newOutputNodeId, result, () =>
            addToHistory(nodesRef.current, edgesRef.current)
          );
  ...
  const handleUpscale = useCallback(
    async (nodeId: string, imageBase64: string, resolution: Resolution) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      ...
      try {
        ...
        updateNodeData<UpscaleNodeData>(
          nodeId,
          {
            isLoading: false,
  ...
  const handleEditNodeGenerateSmartPrompt = useCallback(
    async (nodeId: string) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      ...
      if (
        !editData.designType ||
        (editData.designType !== 'blank' && !editData.uploadedImage) ||
        !editData.tags ||
  ...
  const handleEditNodeSuggestPrompts = useCallback(
    async (nodeId: string) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      if (!node || node.type !== 'edit') return;
