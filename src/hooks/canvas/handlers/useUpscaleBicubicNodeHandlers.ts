...
export const useUpscaleBicubicNodeHandlers = ({
  ...
  const handleUpscaleBicubicApply = useCallback(
    async (nodeId: string, imageInput: string) => {
      ...
      if (!node || node.type !== 'upscaleBicubic') {
        console.warn('handleUpscaleBicubicApply: Node not found or wrong type', {
          nodeId,
          foundNode: !!node,
        });
        return;
  ...
  const handleUpscaleBicubicNodeDataUpdate = useNodeDataUpdateHandler<UpscaleBicubicNodeData>(
    updateNodeData,
    'upscaleBicubic'
  );
