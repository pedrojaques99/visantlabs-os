...
export const usePromptNodeHandlers = ({
  ...
  const handlePromptNodeDataUpdate = useCallback(
    (nodeId: string, newData: Partial<PromptNodeData>) => {
      updateNodeData<PromptNodeData>(nodeId, newData, 'prompt');
    },
    [updateNodeData]
  );

  const handleTextNodeDataUpdate = useCallback(
    (nodeId: string, newData: Partial<TextNodeData>) => {
      updateNodeData<TextNodeData>(nodeId, newData, 'text');
    },
    [updateNodeData]
  );

  const handlePromptSuggestPrompts = useCallback(
    async (nodeId: string, prompt: string) => {
      ...
      updateNodeData<PromptNodeData>(
        nodeId,
        { isSuggestingPrompts: true, promptSuggestions: [] },
        'prompt'
      );
  ...
  const handlePromptGenerate = useCallback(
    async (
      ...
      if (!node || node.type !== 'prompt') {
        console.warn('handlePromptGenerate: Node not found or wrong type', {
          nodeId,
          foundNode: !!node,
        });
        return;
      ...
      trackCanvasEvent('generation_started', 'prompt', undefined, {
        model: selectedModel,
        provider,
      });
      ...
      if (skeletonNode) {
        ...
        console.warn(
          '[handlePromptGenerate] Failed to create OutputNode skeleton - reactFlowInstance may not be available'
        );
      }
      ...
      try {
        ...
        if (connectedImages && connectedImages.length > 0) {
          ...
          if (uploadedImages.length > 0) {
            ...
            const currentPromptData = nodesRef.current.find((n) => n.id === nodeId)
              ?.data as PromptNodeData;
            const isLogoFirst =
              currentPromptData?.connectedLogo &&
              connectedImages?.[0] === currentPromptData.connectedLogo;
            const isIdentitySecond =
              currentPromptData?.connectedIdentity &&
              connectedImages?.[1] === currentPromptData.connectedIdentity;
          ...
          console.log(
            '[handlePromptGenerate] No connected images provided - generating from prompt only'
          );
        }
        ...
        let enhancedPrompt =
          source === 'edge' && tokens ? buildEnhancement(resolvedPrompt, tokens) : resolvedPrompt;
        ...
        console.log('[handlePromptGenerate] Brand context:', {
          source,
          hasEnhancement: enhancedPrompt !== prompt,
        });
        ...
        trackCanvasEvent('generation_completed', 'prompt', undefined, {
          model: selectedModel,
          provider,
        });
