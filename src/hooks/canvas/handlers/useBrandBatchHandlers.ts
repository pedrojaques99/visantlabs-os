...
import type { FlowNodeData, BrandBatchNodeData, BrandBatchItem, ImageNodeData, OutputNodeData, LogoNodeData, BrandCoreData, BrandNodeData } from '@/types/reactFlow';
...
function getConnectedImageUrls(
  ...
  const inEdges = edges.filter(e => e.target === batchNodeId && e.targetHandle?.startsWith('input-'));
...
export function useBrandBatchHandlers({
  ...
  const handleBrandBatchRun = useCallback(async (batchNodeId: string) => {
    ...
    updateNodeData<BrandBatchNodeData>(batchNodeId, {
      status: 'running',
      items: initialItems,
    }, 'brandBatch');
    ...
    const { source: brandSource, tokens: brandTokens } = getBrandContextForNode(
      batchNodeId, nodesRef.current, edgesRef.current, linkedGuideline
    );
    ...
    const runItem = async (i: number) => {
      ...
      try {
        ...
        if (!isSvg) {
          try { isSvg = atob(imageBase64.slice(0, 60)).includes('<svg'); } catch {}
        }
        if (isSvg) {
        ...
        if (nodeData.connectedLogo) {
          ...
          if (!logoIsSvg) {
            ...
            try { logoIsSvg = atob(logoB64.slice(0, 60)).includes('<svg'); } catch {}
            if (!logoIsSvg) {
    ...
    trackCanvasEvent(hadCancel ? 'generation_failed' : 'generation_completed', 'brandBatch', undefined, {
      images: images.length,
      done: doneCount,
      failed: failedCount,
    });
  ...
  const handleBrandBatchCancel = useCallback((batchNodeId: string) => {
