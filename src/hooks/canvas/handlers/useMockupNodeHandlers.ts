...
export const useMockupNodeHandlers = ({
  ...
  const handleMockupNodeDataUpdate = useCallback(
    (nodeId: string, newData: Partial<MockupNodeData>) => {
      updateNodeData<MockupNodeData>(nodeId, newData, 'mockup');
    },
    [updateNodeData]
  );

  const handleMockupGenerate = useCallback(
    async (
      nodeId: string,
      imageInput: string,
      presetId: string,
      ...
      if (!node || node.type !== 'mockup') {
        console.warn('handleMockupGenerate: Node not found or wrong type', {
          nodeId,
          foundNode: !!node,
        });
        return;
      ...
      if (customMockup) {
        ...
        console.log(`[handleMockupGenerate] Loaded preset:`, {
          id: preset.id,
          name: preset.name,
          hasReferenceImageUrl: !!(
            preset.referenceImageUrl && preset.referenceImageUrl.trim() !== ''
          ),
      ...
      } = resolveGenerationContext(model, {
      ...
      setNodes((nds: Node<FlowNodeData>[]) =>
        nds.map((n: Node<FlowNodeData>) =>
          n.id === nodeId
            ? ({
                ...n,
                data: { ...(n.data as MockupNodeData), isLoading: true } as MockupNodeData,
      ...
      if (connectedIdentity) {
        try {
          ...
          if (validateBase64Image(identityBase64)) {
            ...
            console.log(
              '[handleMockupGenerate] Identity set as referenceImage (context/colors/vibe)'
            );
          }
        } catch (error: any) {
          console.warn(
            'Failed to process identity as reference image, continuing without it:',
            error
          );
        }
      }
      ...
      const { source: brandSource, tokens: mockupBrandTokens } = getBrandContextForNode(
        nodeId,
        nodesRef.current,
        edgesRef.current,
        linkedGuideline
      ...
      if (
        withHuman &&
        !enhancedPrompt.toLowerCase().includes('human') &&
        !enhancedPrompt.toLowerCase().includes('person')
      ) {
      ...
      try {
        ...
        const hasIdentityAsReference =
          !!mockupData.connectedIdentity && !!referenceImages && referenceImages.length > 0;
