...
export const useCanvasStrategyHandler = ({
  ...
  const handleStrategyNodeGenerate = useCallback(
    async (nodeId: string, strategyType: string, prompt?: string) => {
      ...
      updateNodeData<StrategyNodeData>(
        nodeId,
        { isGenerating: true, generatingStep: strategyType },
        'strategy'
      );
      ...
      try {
        ...
        if (strategyType === 'all' || strategyType === 'archetypes') {
          const marketResearch = getMarketResearchString(newStrategyData.marketResearch);
          const archetypesResult = await generateArchetypes(
            brandPrompt,
            marketResearch || brandPrompt
          );
        ...
        trackCanvasEvent('generation_failed', 'strategy', undefined, {
          strategyType,
          error: error?.message,
        });
  ...
  const handleStrategyNodeDataUpdate = useCallback(
  ...
  const handleStrategyNodeInitialAnalysis = useCallback(
    async (nodeId: string, promptOverride?: string) => {
      ...
      updateNodeData<StrategyNodeData>(
        nodeId,
        { isGenerating: true, generatingStep: 'marketResearch' },
        'strategy'
      );
  ...
  const handleStrategyNodeGenerateSection = useCallback(
    async (nodeId: string, sectionType: string) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      if (!node || node.type !== 'strategy') return;
      ...
      if (
        ...
        typeof cancelGenerationRef.current[nodeId] === 'boolean'
      ) {
        cancelGenerationRef.current[nodeId] = {};
      }
      ...
      try {
        ...
        switch (sectionType) {
          ...
          case 'competitors': {
            const competitorsResult = await generateCompetitors(
              brandPrompt,
              marketResearch || brandPrompt,
              []
            );
          ...
          case 'references': {
            const referencesResult = await generateReferences(
              brandPrompt,
              marketResearch || brandPrompt,
              newStrategyData.competitors || []
            );
            newStrategyData.references = referencesResult.result;
            break;
          }
          case 'swot': {
            const swotResult = await generateSWOT(
              brandPrompt,
              marketResearch || brandPrompt,
              newStrategyData.competitors || []
            );
          ...
          case 'colorPalettes': {
            const colorPalettesResult = await generateColorPalettes(
              brandPrompt,
              newStrategyData.swot || {},
              newStrategyData.references || []
            );
            newStrategyData.colorPalettes = colorPalettesResult.result;
            break;
          }
          case 'visualElements': {
            const visualElementsResult = await generateVisualElements(
              brandPrompt,
              newStrategyData.colorPalettes || []
            );
          ...
          case 'mockupIdeas': {
            ...
            const mockupData: BrandingData = {
              ...
              mercadoNicho:
                typeof mr === 'object' && mr?.mercadoNicho ? mr.mercadoNicho : undefined,
              publicoAlvo: typeof mr === 'object' && mr?.publicoAlvo ? mr.publicoAlvo : undefined,
              posicionamento:
                typeof mr === 'object' && mr?.posicionamento ? mr.posicionamento : undefined,
        ...
        cancelFlags = cancelGenerationRef.current[nodeId];
        const wasCancelled =
          cancelFlags && typeof cancelFlags === 'object' && cancelFlags[sectionType];
  ...
  const handleStrategyNodeGenerateAll = useCallback(
    async (nodeId: string) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      if (!node || node.type !== 'strategy') return;
      ...
      updateNodeData<StrategyNodeData>(
        nodeId,
        { isGenerating: true, generatingStep: 'all' },
        'strategy'
      );
      ...
      try {
        ...
        const hasSectionData = (sectionType: string): boolean => {
          switch (sectionType) {
            ...
            case 'visualElements':
              return !!(
                newStrategyData.visualElements && newStrategyData.visualElements.length > 0
              );
            case 'mockupIdeas':
            ...
            switch (section.type) {
              ...
              case 'persona': {
                const marketResearch =
                  getMarketResearchString(newStrategyData.marketResearch) || brandPrompt;
              ...
              case 'mockupIdeas': {
                ...
                const allData: BrandingData = {
                  ...
                  mercadoNicho:
                    typeof mr === 'object' && mr?.mercadoNicho ? mr.mercadoNicho : undefined,
                  publicoAlvo:
                    typeof mr === 'object' && mr?.publicoAlvo ? mr.publicoAlvo : undefined,
                  posicionamento:
                    typeof mr === 'object' && mr?.posicionamento ? mr.posicionamento : undefined,
                  insights: typeof mr === 'object' && mr?.insights ? mr.insights : undefined,
                  competitors: newStrategyData.competitors,
  ...
  const handleStrategyNodeCancelGeneration = useCallback(
    (nodeId: string, sectionType?: string) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      if (!node || node.type !== 'strategy') return;
  ...
  const handleStrategyNodeGeneratePDF = useCallback(
    async (nodeId: string) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      if (!node || node.type !== 'strategy') return;
  ...
  const handleStrategyNodeSave = useCallback(
    async (nodeId: string) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      if (!node || node.type !== 'strategy') return;
      ...
      try {
        ...
        updateNodeData<StrategyNodeData>(
          nodeId,
          { projectId: savedProjectId, name: savedProject.name || projectName },
          'strategy'
        );
