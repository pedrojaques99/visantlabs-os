...
export const useCanvasChatHandler = ({
  ...
  const handleChatRemoveNode = useCallback(
    (targetNodeId: string): boolean => {
      ...
      if (setEdges) {
        setEdges((eds: Edge[]) =>
          eds.filter((e) => e.source !== targetNodeId && e.target !== targetNodeId)
        );
      }
      return true;
    },
        ...
        case 'mockup':
          if (creators.addMockupNode) {
            ...
            if (newNodeId && initialData) {
              setTimeout(() => {
                updateNodeData<MockupNodeData>(
                  newNodeId!,
                  initialData as Partial<MockupNodeData>,
                  'mockup'
                );
              }, 50);
            }
  ...
  const handleChatSendMessage = useCallback(
    async (
      ...
      if (canvasCmd) {
        ...
        if (canvasCmd.action === 'create' && canvasCmd.nodeType && canvasCmd.count) {
          ...
          for (let i = 0; i < canvasCmd.count; i++) {
            const id = handleChatCreateNode(
              nodeId,
              canvasCmd.nodeType as FlowNodeType,
              undefined,
              false
            );
          ...
          const typeLabel =
            canvasCmd.nodeType.charAt(0).toUpperCase() + canvasCmd.nodeType.slice(1);
          systemReply =
            createdIds.length > 0
        ...
        if (canvasCmd.action === 'generate-campaign') {
          ...
          if (!productImageUrl) {
            systemReply =
              'Connect a product image to the Chat node first, then ask me to generate ads.';
          } else {
            ...
            systemReply = `Starting campaign generation: ${campaignCount} ads across ${campaignFormats.join(
              ', '
            )} formats. I'll update you when done.`;
            ...
            (async () => {
              try {
                ...
                for (let attempt = 0; attempt < 120; attempt++) {
                  ...
                  const poll = await fetch(`/api/canvas/generate-campaign/${jobId}`, {
                    credentials: 'include',
                  });
                  job = await poll.json();
                ...
                const summary =
                    ...
                    ? `Campaign complete! Generated ${done.length}/${campaignCount} ads.\n\n${done
                        ...
                        .map(
                          (r: any, i: number) =>
                            `${i + 1}. **${r.adAngle}** (${r.format}) — [View](${r.imageUrl})`
                        )
                  ...
                  nodeId,
                  {
                    messages: [
                      ...currentMsgs,
                      {
                        id: `msg-${Date.now()}-err`,
                        role: 'assistant' as const,
                        content: `Campaign failed: ${err.message}`,
                        timestamp: Date.now(),
                      },
                    ],
                  },
        ...
        const currentMessages =
          (nodesRef.current.find((n) => n.id === nodeId)?.data as ChatNodeData)?.messages || [];
        updateNodeData<ChatNodeData>(
          nodeId,
          {
      ...
      const enhancedMessage = chatBrandTokens
        ? buildEnhancement(message.trim(), chatBrandTokens)
        : message.trim();
  ...
  const handleChatUpdateData = useCallback(
    (nodeId: string, newData: Partial<ChatNodeData>) => {
      updateNodeData<ChatNodeData>(nodeId, newData, 'chat');
    },
    [updateNodeData]
  );

  const handleChatClearHistory = useCallback(
    (nodeId: string) => {
      updateNodeData<ChatNodeData>(
        nodeId,
        {
