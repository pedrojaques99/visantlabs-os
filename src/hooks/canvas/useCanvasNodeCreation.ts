...
export const useCanvasNodeCreation = (
    ...
    (customPosition?: { x: number; y: number }): string | undefined => {
      ...
      const newNode: Node<FlowNodeData> = {
        ...
        data: {
          ...
          onGeneratePrompt:
            handlersRef.current?.handleMergeGeneratePrompt || (() => Promise.resolve()),
    ...
    (
      ...
      const newNode: Node<FlowNodeData> = {
        ...
        data: {
          ...
          onSuggestPrompts:
            handlersRef.current?.handlePromptSuggestPrompts || (() => Promise.resolve()),
      ...
      const newNode: Node<FlowNodeData> = {
        ...
        data: {
          ...
          onGenerateSmartPrompt:
            handlersRef.current?.handleEditNodeGenerateSmartPrompt || (() => Promise.resolve()),
          onSuggestPrompts:
            handlersRef.current?.handleEditNodeSuggestPrompts || (() => Promise.resolve()),
  ...
  const addMockupNode = useCallback(
    (customPosition?: { x: number; y: number }, isFlowPosition?: boolean): string | undefined => {
      if (!reactFlowInstance) return;
  ...
  const addAngleNode = useCallback(
    (customPosition?: { x: number; y: number }): string | undefined => {
      if (!reactFlowInstance) return;
  ...
  const addTextureNode = useCallback(
    (customPosition?: { x: number; y: number }): string | undefined => {
      if (!reactFlowInstance) return;
  ...
  const addAmbienceNode = useCallback(
    (customPosition?: { x: number; y: number }): string | undefined => {
      if (!reactFlowInstance) return;
  ...
  const addLuminanceNode = useCallback(
    (customPosition?: { x: number; y: number }): string | undefined => {
      if (!reactFlowInstance) return;
  ...
  const addShaderNode = useCallback(
    (customPosition?: { x: number; y: number }): string | undefined => {
      if (!reactFlowInstance) return;
  ...
  const addTextureFilterNode = useCallback(
    (customPosition?: { x: number; y: number }): string | undefined => {
      if (!reactFlowInstance) return;
  ...
  const addStudio3DNode = useCallback(
    (customPosition?: { x: number; y: number }): string | undefined => {
      if (!reactFlowInstance) return;
  ...
  const addUpscaleBicubicNode = useCallback(
    (customPosition?: { x: number; y: number }): string | undefined => {
      if (!reactFlowInstance) return;
          ...
          if (selectedImageNodes.length > 0) {
            ...
            for (const node of selectedImageNodes) {
              try {
                ...
                const imageUrl = await canvasApi.uploadImageToR2Direct(
                  image.file,
                  canvasId,
                  node.id
                );
                ...
                try {
                  ...
                  console.error(
                    `Fallback to base64 also failed for node ${node.id}:`,
                    fallbackError
                  );
                }
              }
  ...
  const addBrandKitNodes = useCallback(
    (customPosition?: { x: number; y: number }): string[] => {
      if (!reactFlowInstance) return [];
    ...
    (customPosition?: { x: number; y: number }): string | undefined => {
      ...
      const newNode: Node<FlowNodeData> = {
        ...
        data: {
          ...
          onGenerateSection:
            handlersRef.current?.handleStrategyNodeGenerateSection || (() => Promise.resolve()),
          onGenerateAll:
            handlersRef.current?.handleStrategyNodeGenerateAll || (() => Promise.resolve()),
          onInitialAnalysis:
            handlersRef.current?.handleStrategyNodeInitialAnalysis || (() => Promise.resolve()),
      ...
      const newNode: Node<FlowNodeData> = {
        ...
        data: {
          ...
          onGenerateVisualPrompts:
            handlersRef.current?.handleBrandCoreGenerateVisualPrompts || (() => Promise.resolve()),
    ...
    (customPosition?: { x: number; y: number }): string | undefined => {
      ...
      const newNode: Node<FlowNodeData> = {
        ...
        data: {
          type: 'videoInput',
          onUploadVideo:
            handlersRef.current?.handleVideoInputNodeUpload || (() => Promise.resolve()),
    ...
    (
      ...
      if (customPosition && isFlowPosition) {
        ...
        if (
          ...
          !isNaN(customPosition.x) &&
          !isNaN(customPosition.y)
        ) {
