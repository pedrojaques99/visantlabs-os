...
export const useCanvasEvents = (
  ...
  addToHistory: (nodes: Node<FlowNodeData>[], edges: Edge[], drawings?: any) => void,
  ...
  const onConnect = useCallback(
    (params: Connection) => {
        ...
        if (sourceNode?.type === 'text') {
          ...
          if (!targetHandle) {
            const existingEdges = edges.filter(
              (e) => e.target === params.target && e.targetHandle !== 'text-input'
            );
            const availableHandles = ['input-1', 'input-2', 'input-3', 'input-4'].slice(
              0,
              maxHandles
            );
            ...
            if (!targetHandle) {
              ...
              if (existingFromSameSource) {
                targetHandle = existingFromSameSource.targetHandle as
                  | 'input-1'
                  | 'input-2'
                  | 'input-3'
                  | 'input-4';
      ...
      const cleanedParams: Connection = {
        ...
        sourceHandle:
          ...
          params.sourceHandle === ''
            ...
            : params.sourceHandle,
        targetHandle:
          targetHandle === null || targetHandle === 'null' || targetHandle === ''
            ? undefined
            : targetHandle,
      };
      ...
      setEdges((eds) => {
        ...
        if (
          (sourceNode?.type === 'image' || sourceNode?.type === 'output') &&
          targetNode?.type === 'edit'
        ) {
        ...
        if (
          ...
          cleanedParams.targetHandle === 'text-input'
        ) {
        ...
        if (
          (sourceNode?.type === 'image' ||
            sourceNode?.type === 'output' ||
            sourceNode?.type === 'logo') &&
          targetNode?.type === 'colorExtractor' &&
          cleanedParams.targetHandle === 'image-input'
        ) {
          let imageBase64: string | undefined = undefined;
          if (sourceNode.type === 'image') {
            const imageData = sourceNode.data as ImageNodeData;
                  ...
                  ? ({
                      ...
                      data: {
                        ...
                        connectedIdentity:
                          ...
                          brandCoreData.uploadedIdentityUrl ||
                        ...
                        connectedTextDirection:
                          brandCoreData.visualPrompts?.compositionPrompt ||
                          brandCoreData.visualPrompts?.stylePrompt,
    ...
    (
      ...
      targetHandles.forEach((handle) => {
        ...
        if (distance < minDistance) {
          ...
          const handleId =
            handle.getAttribute('data-handleid') || handle.getAttribute('id') || undefined;
  ...
  const onConnectEnd = useCallback(
    (event: MouseEvent | TouchEvent) => {
      ...
      const targetHandle = document
        .elementFromPoint(clientX, clientY)
        ?.closest('.react-flow__handle-target');
      ...
      if (targetNodeElement && !targetHandle) {
        ...
        if (targetNodeId) {
          ...
          if (targetNode && targetNode.id !== sourceNode.id) {
            ...
            if (nearestHandle && nearestHandle.distance < 200) {
      ...
      // BUT: If it's an ImageNode or OutputNode output handle, open context menu instead
      if (!targetNodeElement && !targetHandle) {
        // If connection was started from an ImageNode or OutputNode and dropped on empty canvas
        // Open context menu instead of creating node automatically
        ...
        // For other node types, create node based on handle type
        if (nodeCreators) {
                ...
                setTimeout(() => {
                  ...
                  if (connectionStart.sourceHandle) {
                    ...
                    if (sourceNode) {
                      ...
                      if (
                        !connectionStart.sourceHandle.startsWith('input-') &&
