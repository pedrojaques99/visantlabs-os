...
export const useCanvasNodeHandlers = (
  ...
  const handleUploadImageRef = useRef<
    ((nodeId: string, imageBase64: string) => Promise<void>) | null
  >(null);
  ...
  const uploadImageToR2Auto = useCallback(
    async (
      base64Image: string,
      nodeId: string,
      ...
      return await uploadImageToR2AutoUtil(
        base64Image,
        nodeId,
        canvasId,
        setNodes,
  ...
  const updateOutputNodeWithResult = useCallback(
    (nodeId: string, result: string, addToHistoryCallback: () => void) => {
  ...
  const updateOutputNodeWithR2Url = useCallback(
  ...
  const cleanupFailedNode = useCallback(
    (nodeId: string | null) => {
  ...
  const updateNodeLoadingState = useCallback(
    <T extends FlowNodeData>(nodeId: string, isLoading: boolean, nodeType?: string) => {
  ...
  const updateNodeData = useCallback(
    <T extends FlowNodeData>(nodeId: string, newData: Partial<T>, nodeType?: string) => {
      setNodes((nds: Node<FlowNodeData>[]) =>
        nds.map((n: Node<FlowNodeData>) => {
          if (n.id === nodeId && (!nodeType || n.type === nodeType)) {
            const updatedData = {
            ...
            if (
              ...
              typeof newData.strategyData === 'object'
            ) {
            ...
            if (isLocalDevelopment() && n.type === 'strategy' && newData.strategyData) {
              console.log(`[updateNodeData] Updating strategy node`, {
                ...
                updatedStrategyDataKeys: updatedData.strategyData
                  ? Object.keys(updatedData.strategyData as any)
                  : [],
                updatedStrategyDataCount: updatedData.strategyData
                  ? Object.keys(updatedData.strategyData as any).length
                  : 0,
              });
            }
  ...
  const { handleMockupNodeDataUpdate, handleMockupGenerate } = useMockupNodeHandlers({
    nodesRef,
    edgesRef,
    updateNodeData,
    updateNodeLoadingState,
  ...
  const { handleAngleGenerate, handleAngleNodeDataUpdate } = useAngleNodeHandlers({
    nodesRef,
    edgesRef,
    setNodes,
    setEdges,
  ...
  const { handleTextureGenerate, handleTextureNodeDataUpdate } = useTextureNodeHandlers({
    nodesRef,
    edgesRef,
    setNodes,
    setEdges,
  ...
  const { handleAmbienceGenerate, handleAmbienceNodeDataUpdate } = useAmbienceNodeHandlers({
    nodesRef,
    edgesRef,
    setNodes,
    setEdges,
  ...
  const { handleLuminanceGenerate, handleLuminanceNodeDataUpdate } = useLuminanceNodeHandlers({
    nodesRef,
    edgesRef,
    setNodes,
    setEdges,
  ...
  const handleUpscaleBicubicNodeDataUpdate =
    upscaleBicubicHandlers.handleUpscaleBicubicNodeDataUpdate;
  ...
  const handleBrandNodeDataUpdate = useCallback(
    (nodeId: string, newData: Partial<BrandNodeData>) => {
  ...
  const handleBrandLogoUpload = useCallback(
    (nodeId: string, imageBase64: string) => {
  ...
  const handleBrandPdfUpload = useCallback(
  ...
  const handleColorExtractorUpload = useCallback(
    (nodeId: string, imageBase64: string) => {
  ...
  const handleColorExtractorExtract = useCallback(
    async (nodeId: string, imageBase64: string, shouldRandomize: boolean = false) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      if (!node || node.type !== 'colorExtractor') {
        console.warn('handleColorExtractorExtract: Node not found or wrong type', {
          nodeId,
          foundNode: !!node,
        });
        return;
      ...
      try {
        ...
        updateNodeData<ColorExtractorNodeData>(
          nodeId,
          {
  ...
  const handleColorExtractorRegenerateOne = useCallback(
    async (nodeId: string, imageBase64: string, index: number) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      ...
      try {
        ...
        const newColor = result.colors.find(
          (c) => !otherColors.includes(c) && c !== currentColors[index]
        );
  ...
  const handleColorExtractorNodeDataUpdate = useCallback(
    (nodeId: string, newData: Partial<ColorExtractorNodeData>) => {
  ...
  const handleBrandAnalyze = useCallback(
    async (
      ...
      if (!node || node.type !== 'brand') {
        console.warn('handleBrandAnalyze: Node not found or wrong type', {
          nodeId,
          foundNode: !!node,
        });
        return;
  ...
  // ========== LOGO NODE HANDLERS ==========
  // Handlers para gerenciar operações de node de logo
  const { handleLogoNodeUpload, handleLogoNodeDataUpdate } = useLogoNodeHandlers({
    updateNodeData,
    canvasId,
  });
  ...
  const { handlePDFNodeUpload, handlePDFNodeDataUpdate } = usePDFNodeHandlers({
    updateNodeData,
  });
  ...
  const { handleBatchRun, handleBatchCancel, handleBatchReset, handleBatchNodeDataUpdate } =
  ...
  const handleNodeBuilderSendMessage = useCallback(
  ...
  const handleNodeBuilderSpawn = useCallback(
    (builderNodeId: string, definition: CustomNodeDefinition) => {
          ...
          data: {
            ...
            prompts:
              cfg.renderCategory === 'multi-output'
                ? (cfg as MultiOutputConfig).prompts
                : undefined,
          } as CustomNodeData,
        },
      ]);
  ...
  const handleNodeBuilderUpdateData = useCallback(
    (nodeId: string, newData: Partial<NodeBuilderData>) => {
  ...
  const handleCustomGenerate = useCallback(
    async (
      ...
      try {
        ...
        setEdges((eds: any[]) =>
          eds.filter((e) => e.target !== newOutputNodeId && e.source !== newOutputNodeId)
        );
        throw err;
      }
    },
  ...
  const handleCustomNodeExecute = useCallback(
    async (nodeId: string) => {
      const node = reactFlowInstance?.getNode(nodeId);
      if (!node) return;

      const { definition, prompts, connectedImages, shaderDescription } =
