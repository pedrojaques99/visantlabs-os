...
export const useCanvasKeyboard = (
  ...
  edges: Edge[],
  setNodes: (
    nodes: Node<FlowNodeData>[] | ((nodes: Node<FlowNodeData>[]) => Node<FlowNodeData>[])
  ) => void,
  ...
  useHotkeys(
    ...
    () => {
      ...
      const allDrawingsSelected =
        drawings && drawings.length > 0
          ? drawings.every((d) => selectedDrawingIds?.has(d.id))
          : true;

      const shouldDeselect =
        allNodesSelected && (drawings && drawings.length > 0 ? allDrawingsSelected : true);
