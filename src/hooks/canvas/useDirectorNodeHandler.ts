...
export const useDirectorNodeHandler = ({
   ...
   */
  const handleDirectorAnalyze = useCallback(
    async (nodeId: string) => {
   ...
   */
  const handleDirectorGeneratePrompt = useCallback(
    async (nodeId: string) => {
      const node = nodesRef.current.find((n) => n.id === nodeId);
      if (!node || node.type !== 'director') {
        toast.error('Director node not found');
      ...
      try {
        ...
        if (connectedImage) {
          try {
            ...
            if (isLocalDevelopment()) {
              console.error(
                '[DirectorNode] Error normalizing image for prompt generation:',
                imgErr
              );
            }
        ...
        const { tokens: directorBrandTokens } = getBrandContextForNode(
          nodeId,
          nodesRef.current,
          edgesRef.current,
          linkedGuideline
  ...
  const handleDirectorNodeDataUpdate = useCallback(
    (nodeId: string, newData: Partial<DirectorNodeData>) => {
