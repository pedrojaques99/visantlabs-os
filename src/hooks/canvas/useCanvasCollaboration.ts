...
export const useCanvasCollaboration = ({
  ...
  const isOnlyPositionChange = useCallback(
    (oldNodes: Node<FlowNodeData>[], newNodes: Node<FlowNodeData>[]): boolean => {
  ...
  const isShaderSliderOnlyChange = useCallback(
    (oldNodes: Node<FlowNodeData>[], newNodes: Node<FlowNodeData>[]): boolean => {
      if (oldNodes.length !== newNodes.length) return false;
        ...
        if (oldNode.type === 'shader' && newNode.type === 'shader') {
          ...
          if (sliderChanged) {
            ...
            const strip = (data: any) =>
              Object.fromEntries(
                Object.entries(data).filter(([k]) => !SLIDER_PROPERTIES.includes(k))
              );
            const { position: _op, ...oldRest } = oldNode;
            const { position: _np, ...newRest } = newNode;
            if (
              JSON.stringify({ ...oldRest, data: strip(oldData) }) !==
              JSON.stringify({ ...newRest, data: strip(newData) })
            )
              return false;
          }
        } else {
  ...
  useEffect(() => {
    ...
    const isPositionOnly =
      lastSyncedNodesRef.current.length > 0 &&
    ...
    databaseSyncTimer.current = setTimeout(async () => {
      ...
      if (
        lastSavedNodesRef.current.length > 0 &&
        isShaderSliderOnlyChange(lastSavedNodesRef.current, nodes)
      ) {
      ...
      try {
        ...
        console.error('[canvas] DB save failed:', err);
      }
    }, 2000);
  }, [
    nodes,
  ...
  const isNodeBeingMovedByOthers = useCallback(
    (nodeId: string): { isMoving: boolean; userId?: string; userName?: string } => {
      ...
      for (const other of others) {
        const np = (other.presence as any)?.nodePosition as {
          nodeId: string;
          x: number;
          y: number;
        } | null;
      ...
      handleNodesChangeWithPresence: (changes: any[]) => {
            ...
            if (node) {
              ...
              if (conflict.isMoving) {
                ...
                originalOnNodesChange(
                  changes.map((c: any) =>
                    c.id === posChange.id && c.type === 'position'
        ...
        if (draggingNodeIdRef.current) {
          const posChange = changes.find(
            (c: any) => c.type === 'position' && c.id === draggingNodeIdRef.current && c.position
          );
          if (posChange?.position) {
            ...
            if (
              !last ||
              last.nodeId !== draggingNodeIdRef.current ||
              Math.abs(last.x - x) > 5 ||
              Math.abs(last.y - y) > 5
            ) {
  ...
  if (!isCollaborative || !roomId) {
    return {
      ...
      createPresenceEnhancedHandlers: (
        ...
        handleNodesChangeWithPresence: onNodesChange,
        handleNodeDragStart: onDragStart,
        handleNodeDragStop: onDragStop,
      }),
    };
  }
