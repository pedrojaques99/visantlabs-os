...
export const getBackgroundDescription = (background: string): string => {
  const descriptions: Record<string, string> = {
    'Minimalist Studio':
      'The scene should be set in a professional photography studio with infinite white wall background, studio lighting, clean and minimalist aesthetic.',
    'Light Box':
      'The scene should be set in a professional lightbox photography environment with seamless white or neutral background, even diffused lighting, completely neutral and minimal aesthetic. This is a professional product photography setup with no decorative elements, plants, or distractions - purely focused on showcasing the product with clean, professional lighting.',
    'Grass/Lawn':
      'The scene should be set on a lush green grass lawn with natural outdoor lighting, creating a fresh and organic atmosphere.',
    Concrete:
      'The scene should be set on a textured concrete surface with modern, industrial aesthetic and natural or studio lighting.',
    'Wooden Slat Wall':
      'The scene should be set against a wooden slat wall with warm, natural tones and ambient lighting that highlights the wood texture.',
    'Wooden Table':
      'The scene should be set on a wooden table surface with natural wood grain visible, creating a warm and rustic atmosphere.',
    'Glass Environment':
      'The scene should be set in a modern glass environment with transparent surfaces, reflections, and contemporary lighting.',
    'Modern Office':
      'The scene should be set in a modern office environment with contemporary furniture, clean lines, and professional lighting.',
    'Urban City':
      'The scene should be set in an urban city environment with architectural elements, street aesthetics, and dynamic lighting.',
    'Nature landscape':
      'The scene should be set in a natural landscape environment with organic elements, natural lighting, and outdoor atmosphere.',
    Workspace:
      'The scene should be set in a creative workspace with modern design elements, functional aesthetics, and professional lighting.',
    Tokyo:
      'The scene should be set in a Tokyo-inspired environment with modern Japanese aesthetics, urban elements, and contemporary lighting.',
    'New York':
      'The scene should be set in a New York-inspired environment with urban architecture, street culture, and dynamic lighting.',
    Brazil:
      'The scene should be set in a Brazil-inspired environment with vibrant colors, tropical elements, and warm natural lighting.',
    Paris:
      'The scene should be set in a Paris-inspired environment with elegant architecture, classic aesthetics, and sophisticated lighting.',
    London:
      'The scene should be set in a London-inspired environment with British design elements, urban charm, and atmospheric lighting.',
    Nordic:
      'The scene should be set in a Nordic-inspired environment with minimalist design, natural materials, and soft natural lighting.',
    'California Coast':
      'The scene should be set in a California Coast-inspired environment with coastal aesthetics, natural light, and relaxed atmosphere.',
  };

  return (
    descriptions[background] ||
    'The scene should be set in a professional photography environment with appropriate lighting and aesthetic.'
  );
};
...
export const getLightingDescription = (lighting: string): string => {
  const descriptions: Record<string, string> = {
    ...
    'Night Scene':
      'Use night scene lighting with artificial lights, shadows, and atmospheric ambiance.',
    Cinematic:
      'Use cinematic lighting with dramatic contrasts, moody atmosphere, and film-like quality.',
...
export const getEffectDescription = (effect: string): string => {
  const descriptions: Record<string, string> = {
    ...
    'Vintage Film':
      'Apply vintage film aesthetic with grain, color grading, and nostalgic atmosphere.',
    ...
    'Fish-eye lens':
      'Apply fish-eye lens effect with wide-angle distortion and unique perspective.',
...
export const determineArchetypeFromBranding = (tags: string[]): BrandArchetype | null => {
  ...
  const scores = BRAND_ARCHETYPES.map((archetype) => {
    const matches = archetype.brandingTags.filter((tag) =>
      tags.some(
        (t) =>
          t.toLowerCase().includes(tag.toLowerCase()) || tag.toLowerCase().includes(t.toLowerCase())
      )
    ).length;
