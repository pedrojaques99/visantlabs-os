...
export async function extractColors(
  ...
  const colorMap = new Map<
    string,
    { count: number; r: number; g: number; b: number; h: number; s: number; l: number }
  >();
  let pixelCount = 0;
  ...
  const sortedByVibrancy = [...colorEntries].sort((a, b) => {
    ...
    if (a.l < 0.15 || a.l > 0.9) scoreA *= 0.1;
    if (b.l < 0.15 || b.l > 0.9) scoreB *= 0.1;
  ...
  const sourceForVariation =
    vibrantCandidates.length >= neededVariations * 2 ? vibrantCandidates : sortedByVibrancy;
