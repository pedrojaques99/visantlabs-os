...
export const generateBrandingPDF = (
  ...
  stepsToUse.forEach((step) => {
    ...
    // ═══ Visant v2 Steps (101-110) ═══
    else if (step.id === 101 && typeof content === 'object' && content !== null) {
      const cm = content as any;
      if (cm.centralMessage) {
      ...
      if (cm.pillars && Array.isArray(cm.pillars)) {
        addText('Pilares', 12, true);
        cm.pillars.forEach((p: any) => {
      ...
      if (a.archetypes) {
        a.archetypes.forEach((arch: any) => {
          addText(`${arch.role === 'primary' ? 'Primário' : 'Secundário'}: ${arch.name}`, 12, true);
          if (arch.justification) addFormattedContent(arch.justification, 10);
          yPosition += 5;
        });
      }
      if (a.toneOfVoice) {
      ...
      content.forEach((c: any) => {
        ...
        addText(colorBlock, 11, true);
        if (c.role) addText(`Papel: ${c.role}`, 10);
