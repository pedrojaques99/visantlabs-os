...
export function buildGeminiPromptInstructionsTemplate(params: {
  ...
  const safeAreaInstruction = isLogo
    ? `\n${
        designIntegrityStepNumber + 1
      }. **Safe Area (CRITICAL):** The prompt must instruct the image generator to leave a comfortable 'safe area' or 'breathing room' around the uploaded design, stating that the design must not touch or be clipped by the edges of the mockup surface.`
    : '';
  ...
  const designAnalysisBlock =
    analyzeDesignImage && !isBlankMockup
...
**INSTRUCTIONS:**
Based ${
    isBlankMockup ? '' : "on the user's input and the provided design image, "
...
4.  **Focus on Realism:** After the aspect ratio prefix, describe the scene. Emphasize photorealistic details: ${
    enhanceTexture ? '"authentic materials", "subtle surface details", ' : ''
  }"natural reflections", "professional product photography", "sharp focus".
${locationInstruction}
...
${
  isBlankMockup ? designIntegrityStepNumber + 1 : designIntegrityStepNumber + 2
}. **Negative Prompt:** If the user has provided a negative prompt, end your response with a new sentence: "AVOID THE FOLLOWING: [negative prompt]."
