...
function isValidState(data: any): data is PersistedMockupState {
  ...
  const hasImage =
    !!data.uploadedImage && (isHttpUrl(data.uploadedImage.url) || !!data.uploadedImage.base64);
...
export const saveMockupState = (state: PersistedMockupState): void => {
  try {
    ...
    const sanitizedState: PersistedMockupState = {
      ...
      promptPreview:
        typeof state.promptPreview === 'string'
      ...
      uploadedImage: state.uploadedImage
        ? {
            ...
            base64:
              !isHttpUrl(state.uploadedImage.url) &&
              state.uploadedImage.base64 &&
              state.uploadedImage.base64.length < MAX_IMAGE_BASE64_SIZE_BYTES
                ? state.uploadedImage.base64
                : undefined,
      ...
      referenceImages: (state.referenceImages || [])
        .map((img) => ({
          ...
          base64:
            !isHttpUrl(img.url) && img.base64 && img.base64.length < MAX_IMAGE_BASE64_SIZE_BYTES
              ? img.base64
              : undefined,
    ...
    console.log(
      '[💾 Persistence] Initial size:',
      (sizeInBytes / 1024).toFixed(2) + 'KB',
      'Max allowed:',
      (MAX_STATE_SIZE_BYTES / 1024).toFixed(0) + 'KB'
    );
    ...
    if (sizeInBytes > MAX_STATE_SIZE_BYTES) {
      ...
      console.log(
        '[💾 Persistence] After removing ref base64s:',
        (sizeInBytes / 1024).toFixed(2) + 'KB'
      );
    }
    ...
    if (sizeInBytes > MAX_STATE_SIZE_BYTES && sanitizedState.uploadedImage?.base64) {
      ...
      sizeInBytes = new Blob([serializedState]).size;
      console.log(
        '[💾 Persistence] After removing primary base64:',
        (sizeInBytes / 1024).toFixed(2) + 'KB'
      );
    }
    ...
    if (sizeInBytes > MAX_STATE_SIZE_BYTES) {
      console.warn(
        `[💾 Persistence] ❌ State still too large (${(sizeInBytes / 1024 / 1024).toFixed(
          2
        )}MB) even after slimming, skipping save`
      );
      return;
    }
    ...
    console.log(
      '[💾 Persistence] ✅ State saved successfully',
      (sizeInBytes / 1024).toFixed(2) + 'KB'
    );
  } catch (error) {
    if (
      ...
      console.warn(
        '[💾 Persistence] ⚠️ LocalStorage quota exceeded, attempting minimal CORE state save'
      );
      try {
        ...
        console.error(
          '[💾 Persistence] ❌ Even minimal state failed to save - storage is completely full. Clearing key.'
        );
...
export function loadMockupState(): PersistedMockupState | null {
  try {
    ...
    console.log('[💾 Persistence] loadMockupState: Found state', {
      ...
      ageMinutes: parsed.timestamp
        ? ((Date.now() - parsed.timestamp) / 60000).toFixed(1)
        : 'unknown',
    });
