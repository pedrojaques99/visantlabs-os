/**
 * Docs Markdown Generators — Single Source of Truth
 *
 * Pure functions that generate documentation markdown from live API data.
 * Used by:
 *   - Frontend: "Copy as Markdown" feature (DocsPage)
 *   - Server:   SSR docs page (/api/docs/full) for crawlers & AI agents
 *
 * No React, no browser APIs, no Node APIs — portable across both runtimes.
 */
...
export interface MCPTool {
  ...
  examples?: Array<{ name: string; description?: string; input: any; expectedOutput?: string }>;
  'x-cost'?: 'free' | 'credits';
  'x-category'?: string;
  'x-auth'?: boolean;
}
...
/**
 * Generate all sections as a single markdown document.
 * Used by the server-side rendered docs page.
 */
export function generateFullDocsMarkdown(
  openApiSpec: OpenAPISpec | null,
  mcpSpec: MCPSpec | null,
  platformMcpSpec: MCPSpec | null,
  pricingData: PricingData | null
  ...
  const sections = [
    ...
    generateRestApiMarkdown(openApiSpec),
    generateMcpMarkdown(mcpSpec),
    generateAgentsMarkdown(platformMcpSpec),
    generatePluginMarkdown(),
    generateBrandGuidelinesMarkdown(),
    generateFigmaNodesMarkdown(),
    generateCanvasApiMarkdown(),
    generatePricingMarkdown(pricingData),
  ];
...
function generateAgentsMarkdown(platformMcpSpec?: MCPSpec | null): string {
  ...
  return [
    ...
    '',
    '## Authentication',
    'Two methods: **OAuth 2.1** (recommended for AI agents) or **API Key** (for scripts).',
    '',
    ...
    '',
    'Discovery: `GET /.well-known/oauth-authorization-server` · Scopes: `read`, `write`, `generate`',
    '',
    '### API Key (manual)',
    '- Create in **Settings → API Keys** → `Authorization: Bearer visant_sk_xxx`',
    '',
    '## MCP Connection',
    ...
    '**Settings → Connectors → Add** → enter `/api/mcp` URL → OAuth auto-flow',
    '',
    '### Anthropic API (MCP Connector)',
    '```json',
    '{ "mcp_servers": [{ "type": "url", "url": "<your-endpoint>/api/mcp", "name": "visant" }],',
    '  "tools": [{ "type": "mcp_toolset", "mcp_server_name": "visant" }] }',
    '```',
    'Beta header: `anthropic-beta: mcp-client-2025-11-20`',
    '',
    '### Claude Code CLI',
    '`claude mcp add --transport http visant <your-endpoint>/api/mcp`',
    '',
    ...
    '',
    '## Example Flow',
    '1. Connect via Claude.ai Connector or API key',
    '2. Call `account-usage` to check balance',
    '3. Call `brand-guidelines-list` → `brand-guidelines-get` for brand context',
    '4. Call `mockup-generate` with `brandGuidelineId` and check `_meta.credits_remaining`',
  ].join('\n');
}
...
function generateMcpMarkdown(mcpSpec: MCPSpec | null): string {
  ...
  const lines = [
    ...
    '|--------|-----------|----------|-------|',
    `| Platform MCP | Streamable HTTP | \`/api/mcp\` | ${mcpSpec.tools.length}+ tools |`,
    '| Figma MCP | stdio | `npm run mcp:figma` | 9 (Figma node manipulation) |',
    '',
    '## Connect — Claude.ai / Claude Desktop',
    '',
    '**Settings → Connectors → Add** → enter your `/api/mcp` URL → OAuth auto-flow.',
    '',
    '## Connect — Anthropic API',
    '',
    '```json',
    '{ "mcp_servers": [{ "type": "url", "url": "<endpoint>/api/mcp", "name": "visant", "authorization_token": "visant_sk_xxx" }],',
    '  "tools": [{ "type": "mcp_toolset", "mcp_server_name": "visant" }] }',
    '```',
    '',
    '## Connect — Claude Code CLI',
    '',
    '`claude mcp add --transport http visant <endpoint>/api/mcp`',
    '',
    '## Authentication',
    '',
    '- **OAuth 2.1** (recommended): Automatic via DCR + PKCE S256 — zero config for Claude Connector',
    '- **API Key**: `Authorization: Bearer visant_sk_xxx` — create at Settings → API Keys',
    '',
