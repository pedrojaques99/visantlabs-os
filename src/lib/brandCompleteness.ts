...
export function computeBrandCompleteness(g: BrandGuideline | null | undefined): CompletenessReport {
  ...
  const rules: CompletenessRule[] = [
    // Identity (15)
    {
      ...
      weight: 5,
      group: 'identity',
      passed: filled(g.identity?.name || g.name),
    },
    {
      id: 'tagline',
      label: 'Tagline',
      weight: 4,
      group: 'identity',
      passed: filled(g.identity?.tagline || g.tagline),
    },
    {
      id: 'description',
    ...
    {
      id: 'typography',
      label: 'Pelo menos 1 tipografia',
      weight: 8,
      group: 'visual',
      passed: len(g.typography) >= 1,
    },
    {
      id: 'logo',
      label: 'Pelo menos 1 logo',
      weight: 6,
      group: 'visual',
      passed: len(g.logos) >= 1,
    },
    {
      id: 'logo_variants',
    ...
    {
      id: 'archetype',
      label: 'Pelo menos 1 arquétipo',
      weight: 6,
      group: 'strategy',
      passed: len(g.strategy?.archetypes) >= 1,
    },
    {
      id: 'persona',
    ...
    // Voice & guidelines (15)
    {
      ...
      weight: 5,
      group: 'voice',
      passed: filled(g.guidelines?.voice) || len(g.strategy?.voiceValues) >= 1,
    },
    {
      id: 'voice_dos',
      label: 'Lista de "Do\'s"',
    ...
    {
      id: 'media',
      label: 'Media kit ou knowledge',
      weight: 3,
      group: 'assets',
      passed: len(g.media) >= 1 || len(g.knowledgeFiles) >= 1,
    },
    {
      id: 'public_or_figma',
      label: 'Compartilhamento ou Figma vinculado',
      weight: 2,
      group: 'assets',
      passed: !!g.publicSlug || !!g.figmaFileUrl,
    },
  ];
