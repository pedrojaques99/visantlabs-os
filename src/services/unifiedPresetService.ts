    ...
    try {
      ...
      if (response.ok) {
        ...
        cachedPresets = {
          ...
          ambiencePresets:
            data.ambiencePresets?.length > 0 ? data.ambiencePresets : AMBIENCE_PRESETS,
          luminancePresets:
            data.luminancePresets?.length > 0 ? data.luminancePresets : LUMINANCE_PRESETS,
          mockupPresets: data.mockupPresets?.length > 0 ? data.mockupPresets : MOCKUP_PRESETS,
          brandingPresets:
            data.brandingPresets?.length > 0 ? data.brandingPresets : BRANDING_PRESETS,
      ...
      console.warn(
        '[UnifiedPresetService] Failed to load official presets from API, using fallbacks:',
        error
      );
 ...
 */
export async function getPresetsByType<T extends PresetType>(
  type: T
): Promise<UnifiedPresets[`${T}Presets`]> {
