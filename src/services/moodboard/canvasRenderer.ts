...
function buildTimeline(composition: RenderComposition): {
...
export async function renderComposition(
  ...
  for (let frame = 0; frame < totalFrames; frame++) {
    ...
    if (segment.type === 'slide') {
      ...
      const progressB =
        (frame - segment.startFrame) / Math.round(slides[idxB].durationInSeconds * fps);
      applyTransition(
        ctx,
        images[idxA],
        slides[idxA].preset,
        Math.min(progressA, 1),
        images[idxB],
        slides[idxB].preset,
        Math.min(progressB, 1),
        transitionProgress,
        transition,
        width,
        height
