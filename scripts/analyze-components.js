        ...
        const patterns = [
          ...
          `@/components/${
            comp.relativePath
              .replace(/\.[^.]+$/, '')
              .charAt(0)
              .toUpperCase() + comp.relativePath.replace(/\.[^.]+$/, '').slice(1)
          }['"\\?]`,
...
| **Orphaned** | ${categorized.orphaned.length} (${(
  (categorized.orphaned.length / components.length) *
  100
).toFixed(1)}%) |
...
${analysis
  .slice(0, 30)
  .map((c, i) => `| ${fmt(i + 1)} | **${c.name}** | \`${c.relativePath}\` | **${fmt(c.count)}** |`)
  .join('\n')}
...
${
  categorized.veryFrequent.length > 0
    ? categorized.veryFrequent.map((c) => `- **${c.name}** (${c.count})`).join('\n')
    : '_None_'
}

### 💎 Frequent (11-30) — ${categorized.frequent.length}
${
  categorized.frequent.length > 0
    ? categorized.frequent.map((c) => `- **${c.name}** (${c.count})`).join('\n')
    : '_None_'
}
  ...
  categorized.moderate.length > 0
    ? categorized.moderate
      ...
      (categorized.moderate.length > 20
        ? `\n- _...and ${categorized.moderate.length - 20} more_`
        : '')
    : '_None_'
}
...
${
  categorized.rarely.length > 0
    ? categorized.rarely
        .slice(0, 15)
        .map((c) => `- ${c.name} (${c.count})`)
        .join('\n') +
      (categorized.rarely.length > 15 ? `\n- _...and ${categorized.rarely.length - 15} more_` : '')
    : '_None_'
}
...
${categorized.orphaned
  .slice(0, 5)
  .map((c) => `rm src/components/${c.relativePath}`)
  .join('\n')}
