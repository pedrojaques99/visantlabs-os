...
async function run() {
  ...
  try {
    ...
    await users.createIndex(
      { username: 1 },
      { unique: true, sparse: true, name: 'users_username_sparse' }
    );
    console.log('Created sparse unique index for username');

    await users.createIndex(
      { referralCode: 1 },
      { unique: true, sparse: true, name: 'users_referralCode_sparse' }
    );
    ...
    await brandGuidelines.createIndex(
      { publicSlug: 1 },
      { unique: true, sparse: true, name: 'brand_guidelines_publicSlug_sparse' }
    );
