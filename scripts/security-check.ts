...
const patterns = [
  ...
  { name: 'Dangerously Set Inner HTML', regex: /dangerouslySetInnerHTML/, severity: 'MEDIUM' },
  {
    ...
    regex: /(api_key|secret_key|password)\s*[:=]\s*['"][^'"]+['"]/,
    severity: 'HIGH',
  },
  { name: 'Console Log in Production', regex: /console\.log\(/, severity: 'LOW' },
  { name: 'TODO Comment', regex: /TODO:/, severity: 'INFO' },
  {
    name: 'No Auth Middleware in Route',
    regex:
      /router\.(get|post|put|delete|patch)\s*\(\s*['"][^'"]+['"]\s*,\s*(?!(authMiddleware|verifyToken|ensureAuthenticated))/,
    severity: 'MEDIUM',
  }, // Crude check
];
  ...
  lines.forEach((line, index) => {
    patterns.forEach((pattern) => {
      ...
      if (pattern.regex.test(line)) {
        console.log(
          `[${pattern.severity}] ${pattern.name} found in ${path.relative(rootDir, filePath)}:${
            index + 1
          }`
        );
