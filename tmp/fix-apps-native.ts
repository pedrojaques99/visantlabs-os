...
async function main() {
  ...
  try {
    ...
    console.log(
      `Matched ${result.matchedCount} documents, updated ${result.modifiedCount} documents.`
    );

    const result2 = await collection.updateMany({ updatedAt: null }, { $set: { updatedAt: now } });
