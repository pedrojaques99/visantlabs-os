...
export const EditWorkflowModal: React.FC<EditWorkflowModalProps> = ({
    ...
    return createPortal(
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[10000] flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
            <div className="relative max-w-md w-full bg-zinc-900 border border-zinc-800/60 rounded-xl shadow-2xl flex flex-col overflow-hidden animate-slide-up" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between p-6 border-b border-zinc-800/60 bg-zinc-900/20">
                    <h2 className="text-lg font-semibold text-zinc-200 font-manrope tracking-tight">{t('workflows.edit.title') || 'Edit Workflow'}</h2>
                    <button onClick={onClose} className="p-2 text-zinc-500 hover:text-white transition-all hover:bg-zinc-800/50 rounded-lg">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="p-6 space-y-5">
                    <div className="space-y-2">
                        <label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest ml-1">{t('workflows.edit.name') || 'Name'}</label>
                        <Input value={name} onChange={e => setName(e.target.value)} className="bg-zinc-900/50 border-zinc-800 focus:border-brand-cyan/30" />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest ml-1">{t('workflows.edit.description') || 'Description'}</label>
                        <Textarea
                            value={description}
                            onChange={e => setDescription(e.target.value)}
                            className="bg-zinc-900/50 border-zinc-800 focus:border-brand-cyan/30 h-24 resize-none"
                        />
                    </div>
                    <div className="flex gap-4">
                        <div className="flex-1 space-y-2">
                            <label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest ml-1">{t('workflows.edit.category') || 'Category'}</label>
                            <Select
                                value={category}
                        ...
                        <div className="w-32 space-y-2">
                            <label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest ml-1">{t('workflows.edit.visibility') || 'Visibility'}</label>
                            <button
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest ml-1">{t('workflows.edit.tags') || 'Tags (comma separated)'}</label>
                        <Input value={tags} onChange={e => setTags(e.target.value)} className="bg-zinc-900/50 border-zinc-800 focus:border-brand-cyan/30" />
                    </div>
                </div>
